...
public class CompAbilityDamageBuildingArea : CompAbilityEffect
{
	...
	public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
	{
		...
		{
			if (list[i] is Building building && IsBuildingAffected(building) && building.def.useHitPoints)
			{
				building.HitPoints -= DamageAmountBase;
				if (building.HitPoints < 0 && !building.Destroyed)
				{
					building.Destroy(DestroyMode.KillFinalizeLeavingsOnly);
				}
			}
		}
