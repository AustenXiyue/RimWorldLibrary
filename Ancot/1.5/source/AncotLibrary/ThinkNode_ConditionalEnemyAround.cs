...
public class ThinkNode_ConditionalEnemyAround : ThinkNode_Conditional
{
	...
	protected override bool Satisfied(Pawn pawn)
	{
		...
		{
			IEnumerable<Thing> enumerable = from t in pawn.Map.attackTargetsCache.GetPotentialTargetsFor(pawn)
				where t.Thing.Position.InHorDistOf(pawn.Position, distance)
				select t into x
				select x.Thing;
