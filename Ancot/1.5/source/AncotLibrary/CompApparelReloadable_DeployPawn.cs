...
public class CompApparelReloadable_DeployPawn : CompApparelReloadable
{
	...
	public override IEnumerable<Gizmo> CompGetWornGizmosExtra()
	{
		...
		yield return new Command_Action
		{
			...
			icon = (sortie ? ContentFinder<Texture2D>.Get(Props_Deploy.gizmoIconPath2) : ContentFinder<Texture2D>.Get(Props_Deploy.gizmoIconPath1)),
			action = delegate
			{
				sortie = !sortie;
	...
	public void Deploy()
	{
		...
		for (int i = 0; i < maxCanSpawn; i++)
		{
			...
			{
				pawn.health.AddHediff(Props_Deploy.hediffAddToSpawnPawn, null, null);
			}
