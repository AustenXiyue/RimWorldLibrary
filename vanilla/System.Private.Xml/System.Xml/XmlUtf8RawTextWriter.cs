...
internal class XmlUtf8RawTextWriter : XmlRawWriter
{
	...
	protected unsafe void WriteAttributeTextBlock(char* pSrc, char* pSrcEnd)
	{
		fixed (byte* bufBytes = _bufBytes)
		{
			...
			while (true)
			{
				...
				while (ptr < ptr2 && XmlCharType.IsAttributeValueChar((char)(num = *pSrc)) && num <= 127)
				{
					*ptr = (byte)num;
					ptr++;
					pSrc++;
				...
				case 10:
					...
					{
						ptr = LineFeedEntity(ptr);
					}
					break;
				default:
					if (XmlCharType.IsSurrogate(num))
					{
						ptr = EncodeSurrogate(pSrc, pSrcEnd, ptr);
						pSrc += 2;
					}
					else if (num <= 127 || num >= 65534)
					{
						ptr = InvalidXmlChar(num, ptr, entitize: true);
						pSrc++;
					}
					else
					{
						ptr = EncodeMultibyteUTF8(num, ptr);
						pSrc++;
					}
					continue;
				}
	...
	protected unsafe void RawText(char* pSrcBegin, char* pSrcEnd)
	{
		fixed (byte* bufBytes = _bufBytes)
		{
			...
			while (true)
			{
				byte* ptr3 = ptr + (pSrcEnd - ptr2);
				if (ptr3 > bufBytes + _bufLen)
				{
					ptr3 = bufBytes + _bufLen;
				}
				for (; ptr < ptr3; ptr++)
				{
					if ((num = *ptr2) > 127)
				...
				{
					ptr = EncodeSurrogate(ptr2, pSrcEnd, ptr);
					ptr2 += 2;
				}
				else if (num <= 127 || num >= 65534)
				{
					ptr = InvalidXmlChar(num, ptr, entitize: false);
					ptr2++;
				}
				else
				{
	...
	protected unsafe void WriteRawWithCharChecking(char* pSrcBegin, char* pSrcEnd)
	{
		...
		{
			char* ptr = pSrcBegin;
			byte* ptr2 = bufBytes + _bufPos;
			int num = 0;
			while (true)
			{
				byte* ptr3 = ptr2 + (pSrcEnd - ptr);
				if (ptr3 > bufBytes + _bufLen)
				{
					ptr3 = bufBytes + _bufLen;
				}
				while (ptr2 < ptr3 && XmlCharType.IsTextChar((char)(num = *ptr)) && num <= 127)
				{
					*ptr2 = (byte)num;
					ptr2++;
					ptr++;
				...
				default:
					if (XmlCharType.IsSurrogate(num))
					{
						ptr2 = EncodeSurrogate(ptr, pSrcEnd, ptr2);
						ptr += 2;
					}
					else if (num <= 127 || num >= 65534)
					{
