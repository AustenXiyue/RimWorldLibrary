...
internal class XmlEncodedRawTextWriter : XmlRawWriter
{
	...
	protected unsafe void WriteAttributeTextBlock(char* pSrc, char* pSrcEnd)
	{
		fixed (char* bufChars = _bufChars)
		{
			...
			while (true)
			{
				...
				while (ptr < ptr2 && XmlCharType.IsAttributeValueChar((char)(num = *pSrc)))
				{
					*ptr = (char)num;
					ptr++;
					pSrc++;
	...
	protected unsafe void RawText(char* pSrcBegin, char* pSrcEnd)
	{
		fixed (char* bufChars = _bufChars)
		{
			...
			while (true)
			{
				char* ptr3 = ptr + (pSrcEnd - ptr2);
				if (ptr3 > bufChars + _bufLen)
				{
					ptr3 = bufChars + _bufLen;
				}
				for (; ptr < ptr3; ptr++)
				{
					if ((num = *ptr2) >= 55296)
	...
	protected unsafe void WriteRawWithCharChecking(char* pSrcBegin, char* pSrcEnd)
	{
		...
		{
			char* ptr = pSrcBegin;
			char* ptr2 = bufChars + _bufPos;
			int num = 0;
			while (true)
			{
				char* ptr3 = ptr2 + (pSrcEnd - ptr);
				if (ptr3 > bufChars + _bufLen)
				{
					ptr3 = bufChars + _bufLen;
				}
				while (ptr2 < ptr3 && XmlCharType.IsTextChar((char)(num = *ptr)))
				{
					*ptr2 = (char)num;
					ptr2++;
					ptr++;
