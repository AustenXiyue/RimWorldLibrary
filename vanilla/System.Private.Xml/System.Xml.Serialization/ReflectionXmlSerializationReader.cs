...
internal sealed class ReflectionXmlSerializationReader : XmlSerializationReader
{
	...
	private object WriteLiteralStructMethod(StructMapping structMapping, bool isNullable, bool checkType, string defaultNamespace)
	{
		...
		foreach (MemberMapping memberMapping3 in settableMembers)
		{
			...
			if (member.Source == null)
			{
				...
				if (pi != null && typeof(IList).IsAssignableFrom(pi.PropertyType) && (pi.SetMethod == null || !pi.SetMethod.IsPublic))
				{
					member.Source = delegate(object value)
					{
						IList list2 = (IList)pi.GetValue(o2);
				...
				{
					XmlSerializerNamespaces xmlSerializerNamespaces = new XmlSerializerNamespaces();
					ReflectionXmlSerializationReaderHelper.SetMemberValueDelegate setMemberValueDelegate = GetSetMemberValueDelegate(o2, member.Mapping.Name);
					setMemberValueDelegate(o2, xmlSerializerNamespaces);
			...
			{
				string text2 = elementNameObject as string;
				string[] memberIds = choice.MemberIds;
				foreach (string text3 in memberIds)
				{
					if (text3 == text2)
					{
						object value2 = Enum.Parse(choice.Mapping.TypeDesc.Type, text3);
						SetOrAddValueToMember(o2, value2, choice.MemberInfo);
						break;
					}
				}
			}
		...
		foreach (Member member3 in array2)
		{
			...
			{
				MemberInfo[] member4 = o2.GetType().GetMember(member3.Mapping.Name);
				MemberInfo memberInfo = member4[0];
				object collection = null;
				SetCollectionObjectWithCollectionMember(ref collection, member3.Collection, member3.Mapping.TypeDesc.Type);
				ReflectionXmlSerializationReaderHelper.SetMemberValueDelegate setMemberValueDelegate2 = GetSetMemberValueDelegate(o2, memberInfo.Name);
				setMemberValueDelegate2(o2, collection);
			}
		}
		ReadEndElement();
		return o2;
