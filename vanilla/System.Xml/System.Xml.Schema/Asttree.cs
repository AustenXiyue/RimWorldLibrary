	...
	public void CompileXPath(string xPath, bool isField, XmlNamespaceManager nsmgr)
	{
		...
		for (int j = 0; j < arrayList.Count; j++)
		{
			...
			if (axis2 == null)
			{
				...
				{
					fAxisArray.Add(new ForwardAxis(DoubleLinkAxis.ConvertTree((Axis)axis.Input), isdesorself: false));
				}
				else
				{
					fAxisArray.Add(new ForwardAxis(DoubleLinkAxis.ConvertTree(axis), isdesorself: false));
				}
				continue;
			}
			...
			if (IsSelf(axis) && axis.Input != null)
			{
				fAxisArray.Add(new ForwardAxis(DoubleLinkAxis.ConvertTree((Axis)axis.Input), isdesorself: true));
			}
			else
			{
				fAxisArray.Add(new ForwardAxis(DoubleLinkAxis.ConvertTree(axis), isdesorself: true));
			}
		}
	}
