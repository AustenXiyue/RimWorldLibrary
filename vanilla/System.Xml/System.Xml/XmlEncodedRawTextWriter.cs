...
internal class XmlEncodedRawTextWriter : XmlRawWriter
{
	...
	protected unsafe void WriteRawWithCharChecking(char* pSrcBegin, char* pSrcEnd)
	{
		fixed (char* ptr2 = bufChars)
		{
			...
			while (true)
			{
				...
				while (ptr3 < ptr4 && (xmlCharType.charProperties[num = *ptr] & 0x40) != 0)
				{
					*ptr3 = (char)num;
					ptr3++;
					ptr++;
		...
		fixed (char* ptr = text)
		{
			fixed (char* ptr4 = bufChars)
			{
				...
				while (true)
				{
					...
					while (ptr5 < ptr6 && (xmlCharType.charProperties[num = *ptr2] & 0x40) != 0 && num != stopChar)
					{
						*ptr5 = (char)num;
						ptr5++;
						ptr2++;
		...
		fixed (char* ptr = text)
		{
			fixed (char* ptr4 = bufChars)
			{
				...
				while (true)
				{
					...
					while (ptr5 < ptr6 && (xmlCharType.charProperties[num = *ptr2] & 0x80) != 0 && num != 93)
					{
						*ptr5 = (char)num;
						ptr5++;
						ptr2++;
	...
	protected unsafe int WriteCommentOrPiNoFlush(string text, int index, int count, int stopChar, out bool needWriteNewLine)
	{
		...
		{
			char* num = ptr + index;
			fixed (char* ptr5 = bufChars)
			{
				char* ptr2 = num;
				char* ptr3 = ptr2;
				char* ptr4 = num + count;
				char* ptr6 = ptr5 + bufPos;
				int num2 = 0;
				while (true)
				{
					char* ptr7 = ptr6 + (ptr4 - ptr2);
					if (ptr7 > ptr5 + bufLen)
					{
						ptr7 = ptr5 + bufLen;
					}
					while (ptr6 < ptr7 && (xmlCharType.charProperties[num2 = *ptr2] & 0x40) != 0 && num2 != stopChar)
					{
						*ptr6 = (char)num2;
						ptr6++;
						ptr2++;
	...
	protected unsafe int WriteCDataSectionNoFlush(string text, int index, int count, out bool needWriteNewLine)
	{
		...
		fixed (char* ptr = text)
		{
			...
			fixed (char* ptr5 = bufChars)
			{
				...
				while (true)
				{
					char* ptr7 = ptr6 + (ptr3 - ptr2);
					if (ptr7 > ptr5 + bufLen)
					{
						ptr7 = ptr5 + bufLen;
					}
					while (ptr6 < ptr7 && (xmlCharType.charProperties[num2 = *ptr2] & 0x80) != 0 && num2 != 93)
					{
						*ptr6 = (char)num2;
						ptr6++;
						ptr2++;
