...
internal class XmlTextReaderImpl : XmlReader, IXmlLineInfo, IXmlNamespaceResolver
{
	...
	private void AddDefaultAttributesAndNormalize()
	{
		...
		if (attrCount >= 250)
		{
			...
			Array.Sort(array, DtdDefaultAttributeInfoToNodeDataComparer.Instance);
		}
	...
	private void OnXmlReservedAttribute(NodeData attr)
	{
		...
		localName = XmlConvert.TrimString(attr.StringValue);
		if (!(localName == "preserve"))
		{
	...
	private async Task<bool> ParseXmlDeclarationAsync(bool isTextDecl)
	{
		do
		{
			...
			while (true)
			{
				...
				int wsCount = await EatWhitespacesAsync((xmlDeclState == 0) ? null : sb).ConfigureAwait(continueOnCapturedContext: false);
				if (ps.chars[ps.charPos] == '?')
				{
				...
				if (c != 'e')
				{
					if (c != 's')
					{
						if (c != 'v' || !XmlConvert.StrEqual(ps.chars, ps.charPos, num - ps.charPos, "version") || xmlDeclState != 0)
						{
							goto IL_06a5;
						}
						if (!isTextDecl)
					...
					{
						if (!XmlConvert.StrEqual(ps.chars, ps.charPos, num - ps.charPos, "standalone") || (xmlDeclState != 1 && xmlDeclState != 2) || isTextDecl)
						{
							goto IL_06a5;
						}
						if (!isTextDecl)
				...
				{
					if (!XmlConvert.StrEqual(ps.chars, ps.charPos, num - ps.charPos, "encoding") || (xmlDeclState != 1 && (!isTextDecl || xmlDeclState != 0)))
					{
						goto IL_06a5;
					}
					if (!isTextDecl)
				...
				sb.Append(ps.chars, ps.charPos, num - ps.charPos);
				ps.charPos = num;
				if (ps.chars[ps.charPos] != '=')
				{
	...
	private async Task ParseAttributesAsync()
	{
		...
		while (true)
		{
			...
			while (true)
			{
				byte[] charProperties = xmlCharType.charProperties;
				char c;
				char tmpch0 = (c = chars[pos]);
				int num;
				if ((charProperties[(uint)c] & 1) != 0)
				{
					switch (tmpch0)
					{
					case '\n':
				...
				while (true)
				{
					byte[] charProperties2 = xmlCharType.charProperties;
					char tmpch2 = (c = chars[pos]);
					if ((charProperties2[(uint)c] & 8) != 0)
					{
						pos++;
						continue;
					}
				...
				NodeData attr = AddAttribute(pos, num2);
				...
				attr.SetLineInfo2(ps.LineNo, ps.LinePos);
				char c4;
				while ((xmlCharType.charProperties[(uint)(c4 = chars[pos])] & 0x80) != 0)
				{
					pos++;
				}
				if (c4 == c3)
				...
				{
					await ParseAttributeValueSlowAsync(pos, c3, attr).ConfigureAwait(continueOnCapturedContext: false);
					pos = ps.charPos;
					chars = ps.chars;
				}
	...
	private async Task<bool> _ParseTextAsync(Task<Tuple<int, int, int, bool>> parseTask)
	{
		...
		else
		{
			...
			do
			{
				Tuple<int, int, int, bool> tuple3 = await ParseTextAsync(outOrChars).ConfigureAwait(continueOnCapturedContext: false);
				item = tuple3.Item1;
				item2 = tuple3.Item2;
			...
			while (!fullValue && outOrChars <= 32 && stringBuilder.Length < 4096);
			XmlNodeType nodeType = ((stringBuilder.Length < 4096) ? GetTextNodeType(outOrChars) : XmlNodeType.Text);
			if (nodeType != 0)
			{
	...
	private async Task<Tuple<int, EntityType>> HandleEntityReferenceAsync(bool isInAttributeValue, EntityExpandType expandType)
	{
		...
		try
		{
			endPos = await ParseNameAsync().ConfigureAwait(continueOnCapturedContext: false);
		}
		catch (XmlException)
		{
		...
		int linePos = ps.LinePos;
		string name = nameTable.Add(ps.chars, ps.charPos, endPos - ps.charPos);
		ps.charPos = endPos + 1;
		charRefEndPos = -1;
	...
	private async Task<Tuple<EntityType, int>> ParseNumericCharRefAsync(bool expand, StringBuilder internalSubsetBuilder)
	{
		...
		{
			int num;
			newPos = (num = ParseNumericCharRefInline(ps.charPos, expand, internalSubsetBuilder, out charCount, out entityType));
			num = num;
			if (num != -2)
			{
				break;
	...
	private async Task<int> ParseNamedCharRefAsync(bool expand, StringBuilder internalSubsetBuilder)
	{
		...
		{
			int num;
			int newPos = (num = ParseNamedCharRefInline(ps.charPos, expand, internalSubsetBuilder));
			switch (num)
			{
			case -1:
				return -1;
	...
	private async Task<string> ParseEntityNameAsync()
	{
		...
		string result = nameTable.Add(ps.chars, ps.charPos, endPos - ps.charPos);
		ps.charPos = endPos + 1;
		return result;
	}
