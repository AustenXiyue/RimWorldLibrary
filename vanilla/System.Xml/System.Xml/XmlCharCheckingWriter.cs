...
internal class XmlCharCheckingWriter : XmlWrappingWriter
{
	...
	public override void WriteWhitespace(string ws)
	{
		...
		{
			throw new ArgumentException(Res.GetString("The Whitespace or SignificantWhitespace node can contain only XML white space characters. '{0}' is not an XML white space character.", XmlException.BuildCharExceptionArgs(ws, invCharIndex)));
		}
		if (replaceNewLines)
		{
	...
	private void ValidateQName(string name)
	{
		...
		if (num != name.Length)
		{
			throw new ArgumentException(Res.GetString((num == 0 || (colonOffset > -1 && num == colonOffset + 1)) ? "Name cannot begin with the '{0}' character, hexadecimal value {1}." : "The '{0}' character, hexadecimal value {1}, cannot be included in a name.", XmlException.BuildCharExceptionArgs(name, num)));
		}
	}
