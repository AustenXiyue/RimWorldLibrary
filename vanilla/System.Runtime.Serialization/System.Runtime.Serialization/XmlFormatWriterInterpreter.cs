...
internal class XmlFormatWriterInterpreter
{
	...
	{
		XmlDictionaryString @namespace = dataContract.Namespace;
		...
		if (collectionContract.Kind == CollectionKind.Array)
		{
			...
			if (TryWritePrimitiveArray(collectionContract.UnderlyingType, itemType, () => objLocal, collectionItemName, @namespace))
			{
				return;
			}
			...
			{
				if (!TryWritePrimitive(itemType, null, null, i, @namespace, collectionItemName, 0))
				{
					WriteStartElement(itemType, collectionContract.Namespace, @namespace, collectionItemName, 0);
					array2[0] = i;
		...
		while (enumerator != null && enumerator.MoveNext())
		{
			...
			if (!TryWritePrimitive(returnType, () => currentValue, null, null, @namespace, collectionItemName, 0))
			{
				WriteStartElement(returnType, collectionContract.Namespace, @namespace, collectionItemName, 0);
				if (flag2 || flag)
				{
