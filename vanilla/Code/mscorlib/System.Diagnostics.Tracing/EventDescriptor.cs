...
internal struct EventDescriptor
{
	...
	public EventDescriptor(int id, byte version, byte channel, byte level, byte opcode, int task, long keywords)
	{
		...
		if (id > 65535)
		{
			throw new ArgumentOutOfRangeException("id", Environment.GetResourceString("The ID parameter must be in the range {0} through {1}.", 1, ushort.MaxValue));
		}
		m_traceloggingId = 0;
		m_id = (ushort)id;
		m_version = version;
		...
		if (task > 65535)
		{
			throw new ArgumentOutOfRangeException("task", Environment.GetResourceString("The ID parameter must be in the range {0} through {1}.", 1, ushort.MaxValue));
		}
		m_task = (ushort)task;
	}
