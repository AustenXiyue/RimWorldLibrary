...
internal class EventProvider : IDisposable
{
	...
	internal unsafe void Register(Guid providerGuid)
	{
		...
		try
		{
			uint num = UnsafeNativeMethods.ManifestEtw.EventRegister(ref providerGuid, m_etwCallback, GCHandle.ToIntPtr(m_thisGCHandle).ToPointer(), ref m_regHandle);
			if (num != 0)
			{
	...
	private unsafe void GetSessionInfo(Action<int, long> action)
	{
		...
		while (true)
		{
			...
			fixed (Guid* providerId = &m_providerId)
			{
				try
				{
					num = UnsafeNativeMethods.ManifestEtw.EnumerateTraceGuidsEx(UnsafeNativeMethods.ManifestEtw.TRACE_QUERY_INFO_CLASS.TraceGuidQueryInfo, providerId, sizeof(Guid), ptr, ReturnLength, ref ReturnLength);
				}
			...
			case 0:
			{
				UnsafeNativeMethods.ManifestEtw.TRACE_GUID_INFO* ptr2 = (UnsafeNativeMethods.ManifestEtw.TRACE_GUID_INFO*)ptr;
				UnsafeNativeMethods.ManifestEtw.TRACE_PROVIDER_INSTANCE_INFO* ptr3 = (UnsafeNativeMethods.ManifestEtw.TRACE_PROVIDER_INSTANCE_INFO*)(ptr2 + 1);
				int currentProcessId = (int)Win32Native.GetCurrentProcessId();
				for (int i = 0; i < ptr2->InstanceCount; i++)
				{
					if (ptr3->Pid == currentProcessId)
					{
						UnsafeNativeMethods.ManifestEtw.TRACE_ENABLE_INFO* ptr4 = (UnsafeNativeMethods.ManifestEtw.TRACE_ENABLE_INFO*)(ptr3 + 1);
						for (int j = 0; j < ptr3->EnableCount; j++)
						{
							action(ptr4[j].LoggerId, ptr4[j].MatchAllKeyword);
						}
					}
					if (ptr3->NextOffset != 0)
					{
						byte* ptr5 = (byte*)ptr3;
						ptr3 = (UnsafeNativeMethods.ManifestEtw.TRACE_PROVIDER_INSTANCE_INFO*)(ptr5 + ptr3->NextOffset);
						continue;
					}
					break;
				}
	...
	internal unsafe bool WriteEvent(ref EventDescriptor eventDescriptor, Guid* activityID, Guid* childActivityID, params object[] eventPayload)
	{
		...
		if (IsEnabled(eventDescriptor.Level, eventDescriptor.Keywords))
		{
			...
			byte* dataBuffer = stackalloc byte[(int)(uint)(32 * num2)];
			bool flag = false;
			...
			if (!flag && i < 8)
			{
				...
				fixed (char* ptr2 = (string)list2[0])
				{
					fixed (char* ptr3 = (string)list2[1])
					{
						fixed (char* ptr4 = (string)list2[2])
						{
							fixed (char* ptr5 = (string)list2[3])
							{
								fixed (char* ptr6 = (string)list2[4])
								{
									fixed (char* ptr7 = (string)list2[5])
									{
										fixed (char* ptr8 = (string)list2[6])
										{
											fixed (char* ptr9 = (string)list2[7])
											{
												dataDescriptor = ptr;
												if (list2[0] != null)
												{
													dataDescriptor[list[0]].Ptr = (ulong)ptr2;
												}
												if (list2[1] != null)
												{
													dataDescriptor[list[1]].Ptr = (ulong)ptr3;
												}
												if (list2[2] != null)
												{
													dataDescriptor[list[2]].Ptr = (ulong)ptr4;
												}
												if (list2[3] != null)
												{
													dataDescriptor[list[3]].Ptr = (ulong)ptr5;
												}
												if (list2[4] != null)
												{
													dataDescriptor[list[4]].Ptr = (ulong)ptr6;
												}
												if (list2[5] != null)
												{
													dataDescriptor[list[5]].Ptr = (ulong)ptr7;
												}
												if (list2[6] != null)
												{
													dataDescriptor[list[6]].Ptr = (ulong)ptr8;
												}
												if (list2[7] != null)
												{
													dataDescriptor[list[7]].Ptr = (ulong)ptr9;
												}
			...
			else
			{
				...
				for (int k = 0; k < i; k++)
				{
					...
					{
						fixed (char* ptr10 = (string)list2[k])
						{
							dataDescriptor[list[k]].Ptr = (ulong)ptr10;
						}
					}
					else
					{
						fixed (byte* ptr11 = (byte[])list2[k])
						{
	...
	{
		uint result = UnsafeNativeMethods.ManifestEtw.EventUnregister(m_regHandle);
		m_regHandle = 0L;
		return result;
	}
