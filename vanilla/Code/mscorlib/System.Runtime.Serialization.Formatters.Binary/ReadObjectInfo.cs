...
internal sealed class ReadObjectInfo
{
	...
	internal MemberInfo GetMemberInfo(string name)
	{
		...
		if (isSi)
		{
			throw new SerializationException(Environment.GetResourceString("MemberInfo cannot be obtained for ISerialized Object '{0}'.", string.Concat(objectType, " ", name)));
		}
		if (cache.memberInfos == null)
		{
			throw new SerializationException(Environment.GetResourceString("No MemberInfo for Object {0}.", string.Concat(objectType, " ", name)));
		}
		if (Position(name) != -1)
		{
	...
	internal Type GetType(string name)
	{
		...
		if ((object)type == null)
		{
			throw new SerializationException(Environment.GetResourceString("Types not available for ISerializable object '{0}'.", string.Concat(objectType, " ", name)));
		}
		return type;
	}
