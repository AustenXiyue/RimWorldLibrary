...
internal static class TimeSpanParse
{
	...
	private struct TimeSpanRawInfo
	{
		...
		internal bool ProcessToken(ref TimeSpanToken tok, ref TimeSpanResult result)
		{
			...
			{
				result.SetFailure(ParseFailureKind.Overflow, "The TimeSpan could not be parsed because at least one of the numeric components is out of range or contains too many digits.", null);
				return false;
			}
			if (tok.ttt != TTT.Sep && tok.ttt != TTT.Num)
			{
				result.SetFailure(ParseFailureKind.Format, "String was not recognized as a valid TimeSpan.", null);
				return false;
			}
			switch (tok.ttt)
			{
	...
	private struct TimeSpanResult
	{
		...
		internal Exception GetTimeSpanParseException()
		{
			return m_failure switch
			{
				...
				_ => new FormatException(Environment.GetResourceString("Input string was not in a correct format.")), 
			};
		}
	}
	...
	private struct StringParser
	{
		...
		internal bool TryParse(string input, ref TimeSpanResult result)
		{
			...
			if (input == null)
			{
				result.SetFailure(ParseFailureKind.ArgumentNull, "String reference not set to an instance of a String.", null, "input");
				return false;
			}
			str = input;
			len = input.Length;
			...
			if (flag)
			{
				...
				if (time > 0)
				{
					result.SetFailure(ParseFailureKind.Overflow, "The TimeSpan could not be parsed because at least one of the numeric components is out of range or contains too many digits.");
					return false;
				}
			}
			else if (time < 0)
			...
			if (pos < len)
			{
				result.SetFailure(ParseFailureKind.Format, "String was not recognized as a valid TimeSpan.");
				return false;
			}
	...
	private static bool ProcessTerminal_DHMSF(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
	{
		...
		if (flag3)
		{
			if (!TryTimeToTicks(flag2, raw.numbers[0], raw.numbers[1], raw.numbers[2], raw.numbers[3], raw.numbers[4], out var result2))
			{
				result.SetFailure(ParseFailureKind.Overflow, "The TimeSpan could not be parsed because at least one of the numeric components is out of range or contains too many digits.");
				return false;
			}
	...
	private static bool ProcessTerminal_HMS_F_D(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
	{
		...
		if (num)
		{
			...
			{
				flag2 = true;
				flag3 = TryTimeToTicks(flag2, zero, raw.numbers[0], raw.numbers[1], raw.numbers[2], raw.numbers[3], out result2);
				flag4 = flag4 || !flag3;
			}
			if (!flag3 && raw.FullDHMSMatch(raw.PositiveInvariant))
			{
				flag2 = true;
				flag3 = TryTimeToTicks(flag2, raw.numbers[0], raw.numbers[1], raw.numbers[2], raw.numbers[3], zero, out result2);
				flag4 = flag4 || !flag3;
			}
			if (!flag3 && raw.FullAppCompatMatch(raw.PositiveInvariant))
			{
				flag2 = true;
				flag3 = TryTimeToTicks(flag2, raw.numbers[0], raw.numbers[1], raw.numbers[2], zero, raw.numbers[3], out result2);
				flag4 = flag4 || !flag3;
			}
	...
	private static bool ProcessTerminal_HM_S_D(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
	{
		...
		if (num)
		{
			...
			{
				flag2 = true;
				flag3 = TryTimeToTicks(flag2, zero, raw.numbers[0], raw.numbers[1], raw.numbers[2], zero, out result2);
				flag4 = flag4 || !flag3;
			}
			if (!flag3 && raw.FullDHMMatch(raw.PositiveInvariant))
			{
				flag2 = true;
				flag3 = TryTimeToTicks(flag2, raw.numbers[0], raw.numbers[1], raw.numbers[2], zero, zero, out result2);
				flag4 = flag4 || !flag3;
			}
			if (!flag3 && raw.PartialAppCompatMatch(raw.PositiveInvariant))
			{
				flag2 = true;
				flag3 = TryTimeToTicks(flag2, zero, raw.numbers[0], raw.numbers[1], zero, raw.numbers[2], out result2);
				flag4 = flag4 || !flag3;
			}
	...
	private static bool ProcessTerminal_HM(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
	{
		...
		if (flag3)
		{
			if (!TryTimeToTicks(flag2, zero, raw.numbers[0], raw.numbers[1], zero, zero, out result2))
			{
				result.SetFailure(ParseFailureKind.Overflow, "The TimeSpan could not be parsed because at least one of the numeric components is out of range or contains too many digits.");
				return false;
			}
	...
	private static bool ProcessTerminal_D(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
	{
		...
		if (flag3)
		{
			if (!TryTimeToTicks(flag2, raw.numbers[0], zero, zero, zero, zero, out result2))
			{
				result.SetFailure(ParseFailureKind.Overflow, "The TimeSpan could not be parsed because at least one of the numeric components is out of range or contains too many digits.");
				return false;
			}
	...
	private static bool TryParseExactTimeSpan(string input, string format, IFormatProvider formatProvider, TimeSpanStyles styles, ref TimeSpanResult result)
	{
		...
		if (format.Length == 0)
		{
			result.SetFailure(ParseFailureKind.Format, "Format specifier was invalid.");
			return false;
		}
		if (format.Length == 1)
		{
	...
	private static bool TryParseByFormat(string input, string format, TimeSpanStyles styles, ref TimeSpanResult result)
	{
		...
		for (; i < format.Length; i += returnValue)
		{
			...
			case 'h':
				...
				{
					result.SetFailure(ParseFailureKind.Format, "Input string was not in a correct format.");
					return false;
				}
				flag2 = true;
				break;
