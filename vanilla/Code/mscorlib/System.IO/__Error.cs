...
internal static class __Error
{
	...
	internal static void WinIOError(int errorCode, string maybeFullPath)
	{
		...
		case 183:
			if (displayablePath.Length != 0)
			{
				throw new IOException(Environment.GetResourceString("Cannot create \"{0}\" because a file or directory with the same name already exists.", displayablePath), Win32Native.MakeHRFromErrorCode(errorCode), maybeFullPath);
			}
			break;
		case 206:
		...
		case 32:
			...
			throw new IOException(Environment.GetResourceString("The process cannot access the file '{0}' because it is being used by another process.", displayablePath), Win32Native.MakeHRFromErrorCode(errorCode), maybeFullPath);
		case 80:
			if (displayablePath.Length != 0)
			{
				throw new IOException(Environment.GetResourceString("The file '{0}' already exists.", displayablePath), Win32Native.MakeHRFromErrorCode(errorCode), maybeFullPath);
			}
			break;
		case 995:
