...
internal class StandardTaskContinuation : TaskContinuation
{
	...
	internal override void Run(Task completedTask, bool bCanInlineContinuationTask)
	{
		...
		bool num = (completedTask.IsRanToCompletion ? ((options & TaskContinuationOptions.NotOnRanToCompletion) == 0) : (completedTask.IsCanceled ? ((options & TaskContinuationOptions.NotOnCanceled) == 0) : ((options & TaskContinuationOptions.NotOnFaulted) == 0)));
		Task task = m_task;
		if (num)
		{
