...
internal class SimpleCollator
{
	...
	private int FilterOptions(int i, CompareOptions opt)
	{
		...
		if ((opt & CompareOptions.OrdinalIgnoreCase) != 0)
		{
			i = textInfo.ToLower((char)i);
		}
		if ((opt & CompareOptions.IgnoreCase) != 0)
		{
			i = textInfo.ToLower((char)i);
		}
	...
	{
		return MSCompatUnicodeTable.IsIgnorable(i, (byte)((((opt & (CompareOptions.OrdinalIgnoreCase | CompareOptions.Ordinal)) == 0) ? 1 : 0) + (((opt & CompareOptions.IgnoreSymbols) != 0) ? 2 : 0) + (((opt & CompareOptions.IgnoreNonSpace) != 0) ? 4 : 0)));
	}
	...
	private unsafe int CompareInternal(string s1, int idx1, int len1, string s2, int idx2, int len2, out bool targetConsumed, out bool sourceConsumed, bool skipHeadingExtenders, bool immediateBreakup, ref Context ctx)
	{
		...
		{
			while (idx1 < num3 && GetExtenderType(s1[idx1]) != 0)
			{
				idx1++;
			}
			while (idx2 < num4 && GetExtenderType(s2[idx2]) != 0)
			{
				idx2++;
			}
		}
