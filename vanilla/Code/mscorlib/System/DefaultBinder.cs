	...
	public override PropertyInfo SelectProperty(BindingFlags bindingAttr, PropertyInfo[] match, Type returnType, Type[] indexes, ParameterModifier[] modifiers)
	{
		...
		for (int j = 0; j < array.Length; j++)
		{
			...
			if (returnType != null)
			{
				...
				{
					if (!(returnType.UnderlyingSystemType is RuntimeType) || !CanConvertPrimitive((RuntimeType)returnType.UnderlyingSystemType, (RuntimeType)array[j].PropertyType.UnderlyingSystemType))
					{
						continue;
					}
				}
		...
		default:
		{
			...
			for (int j = 1; j < num; j++)
			{
				int num4 = FindMostSpecificType(array[num3].PropertyType, array[j].PropertyType, returnType);
				if (num4 == 0 && indexes != null)
				{
					num4 = FindMostSpecific(array[num3].GetIndexParameters(), array2, null, array[j].GetIndexParameters(), array2, null, indexes, null);
				}
				if (num4 == 0)
				{
					num4 = FindMostSpecificProperty(array[num3], array[j]);
					if (num4 == 0)
					{
						flag = true;
					}
