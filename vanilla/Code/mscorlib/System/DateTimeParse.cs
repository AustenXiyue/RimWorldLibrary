...
internal static class DateTimeParse
{
	...
	private static bool Lex(DS dps, ref __DTString str, ref DateTimeToken dtok, ref DateTimeRawInfo raw, ref DateTimeResult result, ref DateTimeFormatInfo dtfi, DateTimeStyles styles)
	{
		...
		case TokenType.YearNumberToken:
		{
			...
			if (dps == DS.T_NNt && str.Index < str.len - 1 && str.Value[str.Index] == '.')
			{
			...
			case TokenType.SEP_Pm:
				if (raw.timeMark == TM.NotSet)
				{
					...
					if (dps == DS.D_NN && !ProcessTerminaltState(DS.DX_NN, ref result, ref styles, ref raw, dtfi))
					{
						return false;
					}
					raw.AddNumber(dtok.num);
			...
			case TokenType.SEP_YearSuff:
				...
				{
					result.SetFailure(ParseFailureKind.Format, "String was not recognized as a valid DateTime.", failureMessageFormatArgument);
					return false;
				}
				dtok.dtt = DTT.NumDatesuff;
				dtok.suffix = separatorToken;
