		...
		get
		{
			lock (m_AssetPackInfos)
			{
				foreach (AndroidAssetPackInfo value in m_AssetPackInfos.Values)
				{
					...
					if (value.status != AndroidAssetPackStatus.Canceled && value.status != AndroidAssetPackStatus.Completed && value.status != AndroidAssetPackStatus.Failed && value.status != 0)
					{
						return true;
					}
				}
