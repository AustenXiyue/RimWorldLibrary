...
internal class XmlUtf8RawTextWriter : XmlRawWriter
{
	...
	protected unsafe void WriteRawWithCharChecking(char* pSrcBegin, char* pSrcEnd)
	{
		fixed (byte* ptr2 = bufBytes)
		{
			...
			while (true)
			{
				...
				while (ptr3 < ptr4 && (xmlCharType.charProperties[num = *ptr] & 0x40) != 0 && num <= 127)
				{
					*ptr3 = (byte)num;
					ptr3++;
					ptr++;
		...
		fixed (char* ptr = text)
		{
			fixed (byte* ptr4 = bufBytes)
			{
				...
				while (true)
				{
					...
					while (ptr5 < ptr6 && (xmlCharType.charProperties[num = *ptr2] & 0x40) != 0 && num != stopChar && num <= 127)
					{
						*ptr5 = (byte)num;
						ptr5++;
						ptr2++;
		...
		fixed (char* ptr = text)
		{
			fixed (byte* ptr4 = bufBytes)
			{
				...
				while (true)
				{
					...
					while (ptr5 < ptr6 && (xmlCharType.charProperties[num = *ptr2] & 0x80) != 0 && num != 93 && num <= 127)
					{
						*ptr5 = (byte)num;
						ptr5++;
						ptr2++;
	...
	protected void ValidateContentChars(string chars, string propertyName, bool allowOnlyWhitespace)
	{
		...
		while (true)
		{
			...
			if (!xmlCharType.IsTextChar(chars[num]))
			{
				...
				case '<':
				case ']':
					text = Res.GetString("'{0}', hexadecimal value {1}, is an invalid character.", XmlException.BuildCharExceptionArgs(chars, num));
					break;
				default:
		...
		throw new ArgumentException(Res.GetString("XmlWriterSettings.{0} can contain only valid XML text content characters when XmlWriterSettings.CheckCharacters is true. {1}", propertyName, text));
	}
	...
	protected unsafe int WriteCommentOrPiNoFlush(string text, int index, int count, int stopChar, out bool needWriteNewLine)
	{
		...
		{
			char* num = ptr + index;
			fixed (byte* ptr5 = bufBytes)
			{
				char* ptr2 = num;
				char* ptr3 = ptr2;
				char* ptr4 = num + count;
				byte* ptr6 = ptr5 + bufPos;
				int num2 = 0;
				while (true)
				{
					byte* ptr7 = ptr6 + (ptr4 - ptr2);
					if (ptr7 > ptr5 + bufLen)
					{
						ptr7 = ptr5 + bufLen;
					}
					while (ptr6 < ptr7 && (xmlCharType.charProperties[num2 = *ptr2] & 0x40) != 0 && num2 != stopChar && num2 <= 127)
					{
						*ptr6 = (byte)num2;
						ptr6++;
						ptr2++;
					...
					case 45:
						*ptr6 = 45;
						ptr6++;
						if (num2 == stopChar && (ptr2 + 1 == ptr4 || ptr2[1] == '-'))
						{
							*ptr6 = 32;
							ptr6++;
						}
					...
					default:
						...
						{
							ptr6 = EncodeSurrogate(ptr2, ptr4, ptr6);
							ptr2 += 2;
						}
	...
	protected unsafe int WriteCDataSectionNoFlush(string text, int index, int count, out bool needWriteNewLine)
	{
		...
		fixed (char* ptr = text)
		{
			...
			fixed (byte* ptr5 = bufBytes)
			{
				...
				while (true)
				{
					byte* ptr7 = ptr6 + (ptr3 - ptr2);
					if (ptr7 > ptr5 + bufLen)
					{
						ptr7 = ptr5 + bufLen;
					}
					while (ptr6 < ptr7 && (xmlCharType.charProperties[num2 = *ptr2] & 0x80) != 0 && num2 != 93 && num2 <= 127)
					{
						*ptr6 = (byte)num2;
						ptr6++;
						ptr2++;
					...
					default:
						if (XmlCharType.IsSurrogate(num2))
						{
							ptr6 = EncodeSurrogate(ptr2, ptr3, ptr6);
							ptr2 += 2;
						}
						else if (num2 <= 127 || num2 >= 65534)
						{
