...
internal class DtdParser : IDtdParser
{
	...
	private async Task ParseElementDeclAsync()
	{
		if (await GetTokenAsync(needWhiteSpace: true).ConfigureAwait(continueOnCapturedContext: false) == Token.QName)
		{
			...
			else
			{
				...
				{
					ParticleContentValidator pcv2 = new ParticleContentValidator(XmlSchemaContentType.ElementOnly);
					pcv2.Start();
					pcv2.OpenGroup();
					await ParseElementOnlyContentAsync(pcv2, startParenEntityId).ConfigureAwait(continueOnCapturedContext: false);
					elementDecl.ContentValidator = pcv2.Finish(useDFA: true);
				}
			}
			if (await GetTokenAsync(needWhiteSpace: false).ConfigureAwait(continueOnCapturedContext: false) != Token.GreaterThan)
			{
	...
	private async Task<bool> HandleEntityReferenceAsync(XmlQualifiedName entityName, bool paramEntity, bool inLiteral, bool inAttribute)
	{
		...
		SchemaEntity entity = VerifyEntityReference(entityName, paramEntity, mustBeDeclared: true, inAttribute);
		if (entity == null)
		{
			return false;
		}
		...
		{
			Tuple<int, bool> obj = await readerAdapter.PushEntityAsync(entity).ConfigureAwait(continueOnCapturedContext: false);
			item = obj.Item1;
			if (!obj.Item2)
			{
				return false;
		...
		else
		{
			...
			Tuple<int, bool> obj2 = await readerAdapter.PushEntityAsync(entity).ConfigureAwait(continueOnCapturedContext: false);
			item = obj2.Item1;
			if (!obj2.Item2)
			{
				return false;
