...
internal class XmlWellFormedWriter : XmlWriter
{
	...
	private static Exception InvalidCharsException(string name, int badCharIndex)
	{
		...
		return new ArgumentException(Res.GetString("Invalid name character in '{0}'. The '{1}' character, hexadecimal value {2}, cannot be included in a name.", name, array[0], array[1]));
	}

	private void ThrowInvalidStateTransition(Token token, State currentState)
	{
		string @string = Res.GetString("Token {0} in state {1} would result in an invalid XML document.", tokenName[(int)token], GetStateName(currentState));
	...
	private async Task WriteEndAttributeAsync_SepcialAtt()
	{
		...
		try
		{
			...
			case SpecialAttribute.DefaultXmlns:
			{
				string value = attrValueCache.StringValue;
				if (PushNamespaceExplicit(string.Empty, value))
				{
					...
					{
						await rawWriter.WriteNamespaceDeclarationAsync(string.Empty, value).ConfigureAwait(continueOnCapturedContext: false);
					}
					else
					{
			...
			case SpecialAttribute.PrefixedXmlns:
			{
				...
				if (value == "http://www.w3.org/2000/xmlns/" || (value == "http://www.w3.org/XML/1998/namespace" && curDeclPrefix != "xml"))
				{
					throw new ArgumentException(Res.GetString("Cannot bind to the reserved namespace."));
				}
				if (PushNamespaceExplicit(curDeclPrefix, value))
				{
					...
					{
						await rawWriter.WriteNamespaceDeclarationAsync(curDeclPrefix, value).ConfigureAwait(continueOnCapturedContext: false);
					}
					else
					{
			...
			case SpecialAttribute.XmlSpace:
			{
				...
				else
				{
					if (!(value == "preserve"))
					{
						throw new ArgumentException(Res.GetString("'{0}' is an invalid xml:space value.", value));
					}
					elemScopeStack[elemTop].xmlSpace = XmlSpace.Preserve;
				}
	...
	public override async Task WriteQualifiedNameAsync(string localName, string ns)
	{
		...
		try
		{
			...
			{
				await rawWriter.WriteQualifiedNameAsync(prefix, localName, ns).ConfigureAwait(continueOnCapturedContext: false);
			}
		}
		catch
		{
