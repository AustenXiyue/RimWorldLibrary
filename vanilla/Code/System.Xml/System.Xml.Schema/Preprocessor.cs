...
internal sealed class Preprocessor : BaseProcessor
{
		...
		for (int i = 0; i < schema.Includes.Count; i++)
		{
			...
			case Compositor.Import:
			{
				...
				if (xmlSchema2 != null)
				{
					...
					{
						SendValidationEvent("The namespace attribute '{0}' of an import should be the same value as the targetNamespace '{1}' of the imported schema.", @namespace, xmlSchema2.TargetNamespace, xmlSchemaImport);
					}
				...
				else if (@namespace != null)
				{
					...
					{
						SendValidationEvent("The namespace attribute cannot have empty string as its value.", @namespace, xmlSchemaExternal);
					}
					else
					{
						ParseUri(@namespace, "The Namespace '{0}' is an invalid URI.", xmlSchemaExternal);
					}
				}
				continue;
			}
	...
	private void SetSchemaDefaults(XmlSchema schema)
	{
		...
		{
			if ((schema.BlockDefault & ~(XmlSchemaDerivationMethod.Substitution | XmlSchemaDerivationMethod.Extension | XmlSchemaDerivationMethod.Restriction)) != 0)
			{
		...
		{
			if ((schema.FinalDefault & ~(XmlSchemaDerivationMethod.Extension | XmlSchemaDerivationMethod.Restriction | XmlSchemaDerivationMethod.List | XmlSchemaDerivationMethod.Union)) != 0)
			{
	...
	private void PreprocessElement(XmlSchemaElement element)
	{
		...
		{
			if ((element.Final & ~(XmlSchemaDerivationMethod.Extension | XmlSchemaDerivationMethod.Restriction)) != 0)
			{
	...
	private void PreprocessElementContent(XmlSchemaElement element)
	{
		...
		{
			if ((element.Block & ~(XmlSchemaDerivationMethod.Substitution | XmlSchemaDerivationMethod.Extension | XmlSchemaDerivationMethod.Restriction)) != 0)
			{
	...
	private void PreprocessSimpleType(XmlSchemaSimpleType simpleType, bool local)
	{
		...
		else
		{
			...
			{
				if ((simpleType.Final & ~(XmlSchemaDerivationMethod.Extension | XmlSchemaDerivationMethod.Restriction | XmlSchemaDerivationMethod.List | XmlSchemaDerivationMethod.Union)) != 0)
				{
	...
	private void PreprocessComplexType(XmlSchemaComplexType complexType, bool local)
	{
		...
		else
		{
			...
			{
				if ((complexType.Block & ~(XmlSchemaDerivationMethod.Extension | XmlSchemaDerivationMethod.Restriction)) != 0)
				{
			...
			{
				if ((complexType.Final & ~(XmlSchemaDerivationMethod.Extension | XmlSchemaDerivationMethod.Restriction)) != 0)
				{
	...
	private void ValidateNameAttribute(XmlSchemaObject xso)
	{
		...
		if (num != nameAttribute.Length)
		{
			...
			string @string = Res.GetString("The '{0}' character, hexadecimal value {1}, at position {2} within the name, cannot be included in a name.", array[0], array[1], num);
			SendValidationEvent("Invalid 'name' attribute value '{0}': '{1}'.", nameAttribute, @string, xso);
		}
		else
		{
