...
internal sealed class SchemaCollectionPreprocessor : BaseProcessor
{
	...
	private void BuildRefNamespaces(XmlSchema schema)
	{
		...
		for (int i = 0; i < schema.Includes.Count; i++)
		{
			if (schema.Includes[i] is XmlSchemaImport { Namespace: { } @namespace } && referenceNamespaces[@namespace] == null)
			{
	...
	private void Preprocess(XmlSchema schema, string targetNamespace, Compositor compositor)
	{
		...
		for (int i = 0; i < schema.Includes.Count; i++)
		{
			...
			else
			{
				...
				{
					SendValidationEvent("The Namespace '{0}' is an invalid URI.", @namespace, xmlSchemaExternal);
				}
			}
		}
	...
	private void PreprocessSimpleType(XmlSchemaSimpleType simpleType, bool local)
	{
		...
		else
		{
			...
			{
				if ((simpleType.Final & ~(XmlSchemaDerivationMethod.Restriction | XmlSchemaDerivationMethod.List | XmlSchemaDerivationMethod.Union)) != 0)
				{
