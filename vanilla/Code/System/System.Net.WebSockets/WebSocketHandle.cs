...
internal sealed class WebSocketHandle
{
	...
	private async Task<string> ParseAndValidateConnectResponseAsync(Stream stream, ClientWebSocketOptions options, string expectedSecWebSocketAccept, CancellationToken cancellationToken)
	{
		...
		{
			string value;
			string line = (value = await ReadResponseHeaderLineAsync(stream, cancellationToken).ConfigureAwait(continueOnCapturedContext: false));
			if (string.IsNullOrEmpty(value))
			{
				break;
			}
			...
			ValidateAndTrackHeader("Connection", "Upgrade", text2, headerValue, ref foundConnection);
			ValidateAndTrackHeader("Upgrade", "websocket", text2, headerValue, ref foundUpgrade);
			ValidateAndTrackHeader("Sec-WebSocket-Accept", expectedSecWebSocketAccept, text2, headerValue, ref foundSecWebSocketAccept);
