...
internal sealed class RegexParser
{
	...
	internal static RegexTree Parse(string re, RegexOptions op)
	{
		...
		return new RegexTree(regexParser.ScanRegex(), capslist: (regexParser._capnamelist != null) ? regexParser._capnamelist.ToArray() : null, caps: regexParser._caps, capnumlist: regexParser._capnumlist, captop: regexParser._captop, capnames: regexParser._capnames, opts: op);
	}
	...
	internal RegexNode ScanRegex()
	{
		...
		while (CharsRight() > 0)
		{
			...
			case ')':
				if (EmptyStack())
				{
					throw MakeException(global::SR.GetString("Too many )'s."));
				}
				AddGroup();
				PopGroup();
				PopOptions();
			...
			case '{':
				if (Unit() == null)
				{
					throw MakeException(flag2 ? global::SR.GetString("Nested quantifier {0}.", c.ToString()) : global::SR.GetString("Quantifier {x,y} following nothing."));
				}
				MoveLeft();
				goto IL_02c8;
			default:
				throw MakeException(global::SR.GetString("Internal error in ScanRegex."));
			case ' ':
				continue;
			case '!':
				break;
				...
				while (Unit() != null)
				{
					...
					if (num4 > num5)
					{
						throw MakeException(global::SR.GetString("Illegal {x,y} with x > y."));
					}
		...
		if (!EmptyStack())
		{
			throw MakeException(global::SR.GetString("Not enough )'s."));
		}
		AddGroup();
		return Unit();
	}
	...
	internal RegexCharClass ScanCharClass(bool caseInsensitive, bool scanOnly)
	{
		...
		for (; CharsRight() > 0; flag2 = false)
		{
			...
			else if (c == '\\' && CharsRight() > 0)
			{
				...
				case 'd':
					if (!scanOnly)
					{
						if (flag)
						{
							throw MakeException(global::SR.GetString("Cannot include class \\{0} in character range.", c.ToString()));
						}
			...
			if (flag)
			{
				...
				if (c == '[' && !flag4 && !flag2)
				{
					...
					if (CharsRight() > 0 && RightChar() != ']')
					{
						throw MakeException(global::SR.GetString("A subtraction must be the last element in a character class."));
					}
				}
				else
				{
					if (c2 > c)
					{
						throw MakeException(global::SR.GetString("[x-y] range in reverse order."));
					}
		...
		if (!flag3)
		{
			throw MakeException(global::SR.GetString("Unterminated [] set."));
		}
	...
	internal RegexNode ScanGroupOpen()
	{
		...
		if (CharsRight() != 0)
		{
				...
				if (c3 != '!')
				{
					if (c3 != '=')
					{
						...
						if (c >= '0' && c <= '9')
						{
							...
							{
								throw MakeException(global::SR.GetString("Invalid group name: Group names must begin with a word character."));
							}
							if (num == 0)
							{
								throw MakeException(global::SR.GetString("Capture number cannot be zero."));
							}
						}
						...
						if ((num != -1 || flag) && CharsRight() > 0 && RightChar() == '-')
						{
							...
							if (c >= '0' && c <= '9')
							{
								...
								{
									throw MakeException(global::SR.GetString("Reference to undefined group number {0}.", num2));
								}
							...
							else
							{
								...
								{
									throw MakeException(global::SR.GetString("Reference to undefined group name {0}.", text));
								}
			...
			case '(':
			{
				...
				if (CharsRight() > 0)
				{
					...
					if (c >= '0' && c <= '9')
					{
						...
						if (CharsRight() > 0 && MoveRightGetChar() == ')')
						{
							...
							throw MakeException(global::SR.GetString("(?({0}) ) reference to undefined group.", num4.ToString(CultureInfo.CurrentCulture)));
						}
						throw MakeException(global::SR.GetString("(?({0}) ) malformed.", num4.ToString(CultureInfo.CurrentCulture)));
					}
					if (RegexCharClass.IsWordChar(c))
					{
				...
				if (num5 >= 3 && RightChar(1) == '?')
				{
					...
					{
					case '#':
						throw MakeException(global::SR.GetString("Alternation conditions cannot be comments."));
					case '\'':
						throw MakeException(global::SR.GetString("Alternation conditions do not capture and cannot be named."));
					}
		...
		throw MakeException(global::SR.GetString("Unrecognized grouping construct."));
	}
		...
		if (UseOptionX())
		{
			while (true)
			{
				...
				if (CharsRight() >= 3 && RightChar(2) == '#' && RightChar(1) == '?' && RightChar() == '(')
				{
					...
					if (CharsRight() == 0)
					{
						throw MakeException(global::SR.GetString("Unterminated (?#...) comment."));
					}
					MoveRight();
					continue;
				}
	...
	internal RegexNode ScanBackslash()
	{
		if (CharsRight() == 0)
		{
			throw MakeException(global::SR.GetString("Illegal \\ at end of pattern."));
		}
		char c;
		switch (c = RightChar())
		{
	...
	internal RegexNode ScanBasicBackslash()
	{
		...
		case 'k':
			...
			{
				throw MakeException(global::SR.GetString("Malformed \\k<...> named back reference."));
			}
			c2 = RightChar();
			break;
		case '\'':
		...
		if (flag && c2 >= '0' && c2 <= '9')
		{
			...
			if (CharsRight() > 0 && MoveRightGetChar() == c)
			{
				...
				throw MakeException(global::SR.GetString("Reference to undefined group number {0}.", num.ToString(CultureInfo.CurrentCulture)));
			}
		}
		else if (!flag && c2 >= '1' && c2 <= '9')
		{
			...
			else
			{
				...
				if (num5 <= 9)
				{
					throw MakeException(global::SR.GetString("Reference to undefined group number {0}.", num5.ToString(CultureInfo.CurrentCulture)));
				}
			}
		}
		...
		if (c >= '0' && c <= '9')
		{
			if (!flag && UseOptionE())
			{
				...
				while (CharsRight() > 0 && (c = RightChar()) >= '0' && c <= '9')
				{
					...
					{
						throw MakeException(global::SR.GetString("Capture group numbers must be less than or equal to Int32.MaxValue."));
					}
					num3 = num3 * 10 + num4;
					MoveRight();
					if (IsCaptureSlot(num3))
	...
	internal char ScanHex(int c)
	{
		...
		if (c > 0)
		{
			throw MakeException(global::SR.GetString("Insufficient hexadecimal digits."));
		}
		return (char)num;
	}
	...
	internal char ScanControl()
	{
		if (CharsRight() <= 0)
		{
			throw MakeException(global::SR.GetString("Missing control character."));
		}
		...
		throw MakeException(global::SR.GetString("Unrecognized control character."));
	}
	...
	internal char ScanCharEscape()
	{
		...
		default:
			...
			{
				throw MakeException(global::SR.GetString("Unrecognized escape sequence \\{0}.", c.ToString()));
			}
			return c;
		}
	}
	...
	internal string ParseProperty()
	{
		if (CharsRight() < 3)
		{
			throw MakeException(global::SR.GetString("Incomplete \\p{X} character escape."));
		}
		char c = MoveRightGetChar();
		if (c != '{')
		{
			throw MakeException(global::SR.GetString("Malformed \\p{X} character escape."));
		}
		int num = Textpos();
		while (CharsRight() > 0)
		{
	...
	internal void PopGroup()
	{
		...
		if (_group.Type() == 34 && _group.ChildCount() == 0)
		{
			if (_unit == null)
			{
				throw MakeException(global::SR.GetString("Illegal conditional (?(...)) expression."));
			}
	...
	internal void AddGroup()
	{
		if (_group.Type() == 34 || _group.Type() == 33)
		{
			...
			{
				throw MakeException(global::SR.GetString("Too many | in (?()|)."));
			}
		}
		else
		{
	...
	{
		return new ArgumentException(global::SR.GetString("parsing \"{0}\" - {1}", _pattern, message));
	}
