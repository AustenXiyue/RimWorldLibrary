...
public class Uri : ISpanFormattable, IFormattable, ISerializable
{
	...
	protected Uri(SerializationInfo serializationInfo, StreamingContext streamingContext)
	{
		...
		if (@string.Length != 0)
		{
			CreateThis(@string, dontEscape: false, UriKind.Absolute, default(UriCreationOptions));
			return;
		}
		...
		CreateThis(@string, dontEscape: false, UriKind.Relative, default(UriCreationOptions));
	}
		...
		if (!UserDrivenParsing)
		{
			...
			fixed (char* @string = _string)
			{
				...
				else
				{
					...
					{
						if (schemeName[i] != @string[scheme + i])
						{
							flags |= Flags.SchemeNotCanonical;
						}
					}
					if ((_flags & Flags.AuthorityFound) != Flags.Zero && (scheme + i + 3 >= length || @string[scheme + i + 1] != '/' || @string[scheme + i + 2] != '/'))
					{
						flags |= Flags.SchemeNotCanonical;
					}
				}
				if ((_flags & Flags.HasUserInfo) != Flags.Zero)
				{
					...
					check = CheckCanonical(@string, ref scheme, _info.Offset.Host, '@');
			...
			if (DisablePathAndQueryCanonicalization)
			{
				if (flag)
				{
					_string = string.Concat(_string, _originalUnicodeString.AsSpan(num));
				}
				string string2 = _string;
				if (IsImplicitFile || (flags2 & System.UriSyntaxFlags.MayHaveQuery) == 0)
				{
					scheme = string2.Length;
				}
				else
				{
					scheme = string2.IndexOf('?');
					if (scheme == -1)
					{
						scheme = string2.Length;
					}
				...
				_info.Offset.Fragment = (ushort)string2.Length;
				_info.Offset.End = (ushort)string2.Length;
			}
			else
			{
				if (flag)
				...
				fixed (char* string3 = _string)
				{
					check = ((!IsImplicitFile && (flags2 & (System.UriSyntaxFlags.MayHaveQuery | System.UriSyntaxFlags.MayHaveFragment)) != 0) ? CheckCanonical(string3, ref scheme, length, ((flags2 & System.UriSyntaxFlags.MayHaveQuery) != 0) ? '?' : (_syntax.InFact(System.UriSyntaxFlags.MayHaveFragment) ? '#' : '\ufffe')) : CheckCanonical(string3, ref scheme, length, '\uffff'));
					if ((_flags & Flags.AuthorityFound) != Flags.Zero && (flags2 & System.UriSyntaxFlags.PathIsRooted) != 0 && (_info.Offset.Path == length || (string3[(int)_info.Offset.Path] != '/' && string3[(int)_info.Offset.Path] != '\\')))
					{
				...
				_info.Offset.Query = (ushort)scheme;
				fixed (char* string4 = _string)
				{
					if (scheme < length && string4[scheme] == '?')
					{
						scheme++;
						check = CheckCanonical(string4, ref scheme, length, ((flags2 & System.UriSyntaxFlags.MayHaveFragment) != 0) ? '#' : '\ufffe');
						if ((check & Check.DisplayCanonical) == 0)
						{
				...
				fixed (char* string5 = _string)
				{
					if (scheme < length && string5[scheme] == '#')
					{
						scheme++;
						check = CheckCanonical(string5, ref scheme, length, '\ufffe');
						if ((check & Check.DisplayCanonical) == 0)
						{
	...
	private unsafe int CheckAuthorityHelper(char* pString, int idx, int length, ref System.ParsingError err, ref Flags flags, UriParser syntax, ref string newHost)
	{
		...
		if (c == '[' && syntax.InFact(System.UriSyntaxFlags.AllowIPv6Host) && System.IPv6AddressHelper.IsValid(pString, j + 1, ref i))
		{
			...
			if (flag3)
			{
				newHost = string.Concat(newHost, new ReadOnlySpan<char>(pString + j, i - j));
				flags |= Flags.HostUnicodeNormalized;
				justNormalized = true;
			}
		}
		...
		else if (i < length && pString[i] == ':')
		{
			if (syntax.InFact(System.UriSyntaxFlags.MayHavePort))
			{
				...
				{
					newHost = string.Concat(newHost, new ReadOnlySpan<char>(pString + num3, idx - num3));
				}
			}
			else
			{
	...
	private unsafe void GetCanonicalPath(ref System.Text.ValueStringBuilder dest, UriFormat formatAs)
	{
		...
		if (formatAs == UriFormat.UriEscaped)
		{
			...
			if (!OperatingSystem.IsWindows() && InFact(Flags.BackslashInPath) && _syntax.NotAny(System.UriSyntaxFlags.ConvertPathSlashes) && _syntax.InFact(System.UriSyntaxFlags.FileLikeUri) && !IsImplicitFile)
			{
				...
				dest.Length = length;
				ReadOnlySpan<char> stringToEscape = MemoryMarshal.CreateReadOnlySpan(in valueStringBuilder.GetPinnableReference(), valueStringBuilder.Length);
		...
		if (InFact(Flags.ShouldBeCompressed) && dest.Length - num > 0)
		{
			...
			if (formatAs == UriFormat.UriEscaped && NotAny(Flags.UserEscaped) && InFact(Flags.E_PathNotCanonical))
			{
				...
				dest.Length = length;
				ReadOnlySpan<char> stringToEscape2 = MemoryMarshal.CreateReadOnlySpan(in valueStringBuilder2.GetPinnableReference(), valueStringBuilder2.Length);
	...
	private static string CombineUri(Uri basePart, string relativePart, UriFormat uriFormat)
	{
		...
		if (c == '/' || (c == '\\' && flag))
		{
			...
			return string.Concat(text, "/", relativePart.AsSpan(1));
		}
	...
	internal unsafe bool InternalIsWellFormedOriginalString()
	{
		...
		fixed (char* @string = _string)
		{
			...
			if (!IsAbsoluteUri)
			{
				...
				return (CheckCanonical(@string, ref idx, _string.Length, '\ufffe') & (Check.EscapedCanonical | Check.BackslashInPath)) == Check.EscapedCanonical;
			}
			if (IsImplicitFile)
			{
				return false;
			...
			if ((_flags & Flags.CanonicalDnsHost) == Flags.Zero && HostType != Flags.IPv6HostType)
			{
				...
				Check check = CheckCanonical(@string, ref idx, _info.Offset.Path, '/');
			...
			if ((_flags & (Flags.SchemeNotCanonical | Flags.AuthorityFound)) == (Flags.SchemeNotCanonical | Flags.AuthorityFound))
			{
				...
				if (idx + 1 >= _string.Length || @string[idx] != '/' || @string[idx + 1] != '/')
				{
					return false;
				}
			}
