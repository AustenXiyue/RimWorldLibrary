...
public sealed class Vector3DCollection : Freezable, IFormattable, IList, ICollection, IEnumerable, IList<Vector3D>, ICollection<Vector3D>, IEnumerable<Vector3D>
{
	...
	bool IList.IsReadOnly => ((ICollection<Vector3D>)this).IsReadOnly;
	...
	private Vector3D Cast(object value)
	{
		...
		if (!(value is Vector3D))
		{
			throw new ArgumentException(SR.Format(SR.Collection_BadType, GetType().Name, value.GetType().Name, "Vector3D"));
		}
	...
	protected override void CloneCore(Freezable source)
	{
		...
		for (int i = 0; i < count; i++)
		{
			_collection.Add(vector3DCollection._collection[i]);
		}
	}
	...
	{
		_collection = default(FrugalStructList<Vector3D>);
	}
	...
	public Vector3DCollection(IEnumerable<Vector3D> collection)
	{
		...
		if (collection != null)
		{
			if (collection is ICollection<Vector3D> collection2)
			{
				_collection = new FrugalStructList<Vector3D>(collection2);
			}
			else if (collection is ICollection collection3)
			{
				_collection = new FrugalStructList<Vector3D>(collection3);
			}
			else
			{
				_collection = default(FrugalStructList<Vector3D>);
