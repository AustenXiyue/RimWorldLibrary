...
public sealed class PathFigureCollection : Animatable, IList, ICollection, IEnumerable, IList<PathFigure>, ICollection<PathFigure>, IEnumerable<PathFigure>, IFormattable
{
	...
	bool IList.IsReadOnly => ((ICollection<PathFigure>)this).IsReadOnly;
	...
	{
		bool flag = true;
		for (int i = 0; i < _collection.Count && flag; i++)
		{
			flag &= _collection[i].CanSerializeToString();
		}
		return flag;
	}
	...
	public bool Remove(PathFigure value)
	{
		...
		if (num >= 0)
		{
			PathFigure oldValue = _collection[num];
	...
	private PathFigure Cast(object value)
	{
		...
		{
			throw new ArgumentException(SR.Format(SR.Collection_BadType, GetType().Name, value.GetType().Name, "PathFigure"));
		}
	...
	protected override void CloneCore(Freezable source)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			OnFreezablePropertyChanged(null, pathFigure);
			_collection.Add(pathFigure);
		}
	}
	...
	{
		_collection = default(FrugalStructList<PathFigure>);
	}
	...
	public PathFigureCollection(IEnumerable<PathFigure> collection)
	{
		...
		{
			bool flag = true;
			if (collection is ICollection<PathFigure> collection2)
			{
				_collection = new FrugalStructList<PathFigure>(collection2);
			}
			else if (collection is ICollection collection3)
			{
				_collection = new FrugalStructList<PathFigure>(collection3);
			}
			else
			{
				_collection = default(FrugalStructList<PathFigure>);
				...
				{
					PathFigure pathFigure = item ?? throw new ArgumentException(SR.Collection_NoNull);
					OnFreezablePropertyChanged(null, pathFigure);
					_collection.Add(pathFigure);
				}
				flag = false;
			...
			if (flag)
			{
				foreach (PathFigure item2 in collection)
				{
					if (item2 == null)
					{
						throw new ArgumentException(SR.Collection_NoNull);
