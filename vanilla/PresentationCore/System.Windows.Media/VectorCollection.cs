...
public sealed class VectorCollection : Freezable, IFormattable, IList, ICollection, IEnumerable, IList<Vector>, ICollection<Vector>, IEnumerable<Vector>
{
	...
	bool IList.IsReadOnly => ((ICollection<Vector>)this).IsReadOnly;
	...
	private Vector Cast(object value)
	{
		...
		if (!(value is Vector))
		{
			throw new ArgumentException(SR.Format(SR.Collection_BadType, GetType().Name, value.GetType().Name, "Vector"));
		}
		return (Vector)value;
	}
	...
	protected override void CloneCore(Freezable source)
	{
		...
		for (int i = 0; i < count; i++)
		{
			_collection.Add(vectorCollection._collection[i]);
		}
	}
	...
	{
		_collection = default(FrugalStructList<Vector>);
	}
	...
	public VectorCollection(IEnumerable<Vector> collection)
	{
		...
		if (collection != null)
		{
			if (collection is ICollection<Vector> collection2)
			{
				_collection = new FrugalStructList<Vector>(collection2);
			}
			else if (collection is ICollection collection3)
			{
				_collection = new FrugalStructList<Vector>(collection3);
			}
			else
			{
				_collection = default(FrugalStructList<Vector>);
