...
public abstract class BitmapSource : ImageSource, DUCE.IResource
{
	...
	internal interface IWICBitmapSource
	{
		...
		[PreserveSig]
		int GetPalette(nint pIPalette);

		[PreserveSig]
		int CopyPixels(nint prc, int cbStride, int cbPixels, nint pvPixels);
	}
	...
	internal class ManagedBitmapSource : IWICBitmapSource
	{
		...
		{
			if (_bitmapSource.TryGetTarget(out var target))
			{
		...
		int IWICBitmapSource.GetPalette(nint pIPalette)
		{
			if (_bitmapSource.TryGetTarget(out var target))
			{
		...
		int IWICBitmapSource.CopyPixels(nint prc, int cbStride, int cbPixels, nint pvPixels)
		{
			if (cbStride < 0)
			{
				return -2147024809;
			...
			if (_bitmapSource.TryGetTarget(out var target))
			{
				Int32Rect sourceRect = ((prc != IntPtr.Zero) ? Marshal.PtrToStructure<Int32Rect>(prc) : new Int32Rect(0, 0, target.PixelWidth, target.PixelHeight));
				...
				checked
				{
					...
					array = new byte[height * num];
					target.CopyPixels(sourceRect, array, num, 0);
				}
				long num2 = ((IntPtr)pvPixels).ToInt64();
				for (int i = 0; i < height; i++)
				{
					Marshal.Copy(array, i * num, new IntPtr(num2), num);
					num2 += cbStride;
				}
				return 0;
			}
			...
			if (_convertedDUCEPtr == null || _convertedDUCEPtr.IsInvalid)
			{
				if (UsableWithoutCache)
				{
					...
					try
					{
						...
						{
							void* pvPixels = ptr;
							HRESULT.Check(UnsafeNativeMethods.WICBitmapSource.CopyPixels(ppIBitmap, ref prc, (uint)num, (uint)num, (nint)pvPixels));
						}
					}
					catch (Exception e)
					{
	...
	public static BitmapSource Create(int pixelWidth, int pixelHeight, double dpiX, double dpiY, PixelFormat pixelFormat, BitmapPalette palette, nint buffer, int bufferSize, int stride)
	{
	...
	internal unsafe void CriticalCopyPixels(Int32Rect sourceRect, Array pixels, int stride, int offset)
	{
		...
		if (pixels is byte[])
		{
			...
			{
				void* buffer = ptr;
				CriticalCopyPixels(sourceRect, (nint)buffer, bufferSize, stride);
			}
		}
		else if (pixels is short[])
		{
			...
			{
				void* buffer2 = ptr2;
				CriticalCopyPixels(sourceRect, (nint)buffer2, bufferSize, stride);
			}
		}
		else if (pixels is ushort[])
		{
			...
			{
				void* buffer3 = ptr3;
				CriticalCopyPixels(sourceRect, (nint)buffer3, bufferSize, stride);
			}
		}
		else if (pixels is int[])
		{
			...
			{
				void* buffer4 = ptr4;
				CriticalCopyPixels(sourceRect, (nint)buffer4, bufferSize, stride);
			}
		}
		else if (pixels is uint[])
		{
			...
			{
				void* buffer5 = ptr5;
				CriticalCopyPixels(sourceRect, (nint)buffer5, bufferSize, stride);
			}
		}
		else if (pixels is float[])
		{
			...
			{
				void* buffer6 = ptr6;
				CriticalCopyPixels(sourceRect, (nint)buffer6, bufferSize, stride);
			}
		}
		else if (pixels is double[])
		{
			...
			{
				void* buffer7 = ptr7;
				CriticalCopyPixels(sourceRect, (nint)buffer7, bufferSize, stride);
			}
		}
	}

	internal void CriticalCopyPixels(Int32Rect sourceRect, nint buffer, int bufferSize, int stride)
	{
		...
		if (stride < num)
		{
			throw new ArgumentOutOfRangeException("stride", SR.Format(SR.ParameterCannotBeLessThan, num));
		}
		int num2 = checked(stride * (sourceRect.Height - 1) + num);
		if (bufferSize < num2)
		{
			throw new ArgumentOutOfRangeException("buffer", SR.Format(SR.ParameterCannotBeLessThan, num2));
		}
		lock (_syncObject)
		{
	...
	internal static BitmapSourceSafeMILHandle CreateCachedBitmap(BitmapFrame frame, BitmapSourceSafeMILHandle wicSource, BitmapCreateOptions createOptions, BitmapCacheOption cacheOption, BitmapPalette palette)
	{
		...
		if ((createOptions & BitmapCreateOptions.PreservePixelFormat) == 0)
		{
		...
		if (frame != null && (createOptions & BitmapCreateOptions.IgnoreColorProfile) == 0 && frame.ColorContexts != null && frame.ColorContexts[0] != null && frame.ColorContexts[0].IsValid && !frame._isColorCorrected && PixelFormat.GetPixelFormat(wicSource).Format != 0)
		{
			...
			if (colorContext != null)
			{
				...
				try
				{
					wicSource = new ColorConvertedBitmap(frame, frame.ColorContexts[0], colorContext, pixelFormat).WicSourceHandle;
				...
				{
					flag = false;
					wicSource = new ColorConvertedBitmap(new FormatConvertedBitmap(frame, pixelFormat, null, 0.0), frame.ColorContexts[0], colorContext, pixelFormat).WicSourceHandle;
					frame._isColorCorrected = true;
					flag = false;
				}
			}
	...
	private void OnSourceDownloadCompleted(object sender, EventArgs e)
	{
		...
		if (_bitmapInit.IsInitAtLeastOnce && IsValidForFinalizeCreation(throwIfInvalid: false))
		{
			try
			{
				FinalizeCreation();
