...
public class Point3DAnimation : Point3DAnimationBase
{
	...
	protected override Point3D GetCurrentValueCore(Point3D defaultOriginValue, Point3D defaultDestinationValue, AnimationClock animationClock)
	{
		...
		case AnimationType.From:
			point3D = _keyValues[0];
			point3D2 = defaultDestinationValue;
			flag2 = true;
			break;
		case AnimationType.To:
			point3D = defaultOriginValue;
			point3D2 = _keyValues[0];
			flag = true;
			break;
		...
		case AnimationType.FromBy:
			point3D = _keyValues[0];
			point3D2 = AnimatedTypeHelpers.AddPoint3D(_keyValues[0], _keyValues[1]);
			if (IsAdditive)
			{
				value2 = defaultOriginValue;
				flag = true;
		...
		if (IsCumulative)
		{
			...
			if (num2 > 0.0)
			{
				value = AnimatedTypeHelpers.ScalePoint3D(AnimatedTypeHelpers.SubtractPoint3D(point3D2, point3D), num2);
			}
		}
	...
	{
		Point3DAnimation obj = (Point3DAnimation)d;
	...
	private static bool ValidateFromToOrByValue(object value)
	{
		...
		if (point3D.HasValue)
		{
			return AnimatedTypeHelpers.IsValidAnimationValuePoint3D(point3D.Value);
		}
		return true;
	}
}
