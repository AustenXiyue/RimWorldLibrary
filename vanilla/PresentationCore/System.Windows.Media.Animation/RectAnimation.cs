...
public class RectAnimation : RectAnimationBase
{
	...
	protected override Rect GetCurrentValueCore(Rect defaultOriginValue, Rect defaultDestinationValue, AnimationClock animationClock)
	{
		...
		case AnimationType.FromBy:
			rect = _keyValues[0];
			rect2 = AnimatedTypeHelpers.AddRect(_keyValues[0], _keyValues[1]);
			if (IsAdditive)
			{
				value2 = defaultOriginValue;
				flag = true;
		...
		if (IsCumulative)
		{
			...
			if (num2 > 0.0)
			{
				value = AnimatedTypeHelpers.ScaleRect(AnimatedTypeHelpers.SubtractRect(rect2, rect), num2);
			}
		}
	...
	{
		RectAnimation obj = (RectAnimation)d;
	...
	private static bool ValidateFromToOrByValue(object value)
	{
		...
		if (rect.HasValue)
		{
			return AnimatedTypeHelpers.IsValidAnimationValueRect(rect.Value);
		}
		return true;
	}
}
