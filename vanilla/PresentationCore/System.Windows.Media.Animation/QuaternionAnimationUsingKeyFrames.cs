...
public class QuaternionAnimationUsingKeyFrames : QuaternionAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override Quaternion GetCurrentValueCore(Quaternion defaultOriginValue, Quaternion defaultDestinationValue, AnimationClock animationClock)
	{
		...
		if (i == num)
		{
			quaternion = GetResolvedKeyFrameValue(num2);
		}
		else if (value == _sortedResolvedKeyFrames[i]._resolvedKeyTime)
		{
			quaternion = GetResolvedKeyFrameValue(i);
		}
		else
		{
			double num3 = 0.0;
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValueQuaternion(defaultOriginValue));
			...
			quaternion = GetResolvedKeyFrame(i).InterpolateValue(baseValue, num3);
		}
		if (IsCumulative)
		{
			...
			if (num5 > 0.0)
			{
				quaternion = AnimatedTypeHelpers.AddQuaternion(quaternion, AnimatedTypeHelpers.ScaleQuaternion(GetResolvedKeyFrameValue(num2), num5));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				Quaternion from = _keyFrames[num - 1].Value;
				do
				{
					Quaternion value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthQuaternion(from, _keyFrames[num].Value);
