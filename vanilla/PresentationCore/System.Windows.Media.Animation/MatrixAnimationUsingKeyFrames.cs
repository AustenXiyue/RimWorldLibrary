...
public class MatrixAnimationUsingKeyFrames : MatrixAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override Matrix GetCurrentValueCore(Matrix defaultOriginValue, Matrix defaultDestinationValue, AnimationClock animationClock)
	{
		...
		if (i == num)
		{
			return GetResolvedKeyFrameValue(num2);
		}
		if (value == _sortedResolvedKeyFrames[i]._resolvedKeyTime)
		{
			return GetResolvedKeyFrameValue(i);
		}
		double num3 = 0.0;
		Matrix baseValue;
		if (i == 0)
		...
		return GetResolvedKeyFrame(i).InterpolateValue(baseValue, num3);
	}
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				Matrix from = _keyFrames[num - 1].Value;
				do
				{
					Matrix value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthMatrix(from, _keyFrames[num].Value);
