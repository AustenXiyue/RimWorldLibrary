...
public class Int64AnimationUsingKeyFrames : Int64AnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override long GetCurrentValueCore(long defaultOriginValue, long defaultDestinationValue, AnimationClock animationClock)
	{
		...
		else
		{
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValueInt64(defaultOriginValue));
		...
		if (IsCumulative)
		{
			...
			if (num6 > 0.0)
			{
				num3 = AnimatedTypeHelpers.AddInt64(num3, AnimatedTypeHelpers.ScaleInt64(GetResolvedKeyFrameValue(num2), num6));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				long from = _keyFrames[num - 1].Value;
				do
				{
					long value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthInt64(from, _keyFrames[num].Value);
