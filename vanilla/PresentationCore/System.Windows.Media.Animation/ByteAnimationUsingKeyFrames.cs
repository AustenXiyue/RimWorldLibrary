...
public class ByteAnimationUsingKeyFrames : ByteAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override byte GetCurrentValueCore(byte defaultOriginValue, byte defaultDestinationValue, AnimationClock animationClock)
	{
		...
		if (i == num)
		{
			b = GetResolvedKeyFrameValue(num2);
		}
		else if (value == _sortedResolvedKeyFrames[i]._resolvedKeyTime)
		{
			b = GetResolvedKeyFrameValue(i);
		}
		else
		{
			double num3 = 0.0;
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValueByte(defaultOriginValue));
			...
			b = GetResolvedKeyFrame(i).InterpolateValue(baseValue, num3);
		}
		if (IsCumulative)
		{
			...
			if (num5 > 0.0)
			{
				b = AnimatedTypeHelpers.AddByte(b, AnimatedTypeHelpers.ScaleByte(GetResolvedKeyFrameValue(num2), num5));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				byte from = _keyFrames[num - 1].Value;
				do
				{
					byte value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthByte(from, _keyFrames[num].Value);
