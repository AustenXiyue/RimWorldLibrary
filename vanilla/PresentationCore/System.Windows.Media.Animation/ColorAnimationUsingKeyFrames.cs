...
public class ColorAnimationUsingKeyFrames : ColorAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override Color GetCurrentValueCore(Color defaultOriginValue, Color defaultDestinationValue, AnimationClock animationClock)
	{
		...
		if (i == num)
		{
			color = GetResolvedKeyFrameValue(num2);
		}
		else if (value == _sortedResolvedKeyFrames[i]._resolvedKeyTime)
		{
			color = GetResolvedKeyFrameValue(i);
		}
		else
		{
			double num3 = 0.0;
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValueColor(defaultOriginValue));
			...
			color = GetResolvedKeyFrame(i).InterpolateValue(baseValue, num3);
		}
		if (IsCumulative)
		{
			...
			if (num5 > 0.0)
			{
				color = AnimatedTypeHelpers.AddColor(color, AnimatedTypeHelpers.ScaleColor(GetResolvedKeyFrameValue(num2), num5));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				Color from = _keyFrames[num - 1].Value;
				do
				{
					Color value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthColor(from, _keyFrames[num].Value);
