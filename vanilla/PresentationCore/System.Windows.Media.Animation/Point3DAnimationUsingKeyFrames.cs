...
public class Point3DAnimationUsingKeyFrames : Point3DAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override Point3D GetCurrentValueCore(Point3D defaultOriginValue, Point3D defaultDestinationValue, AnimationClock animationClock)
	{
		...
		if (i == num)
		{
			point3D = GetResolvedKeyFrameValue(num2);
		}
		else if (value == _sortedResolvedKeyFrames[i]._resolvedKeyTime)
		{
			point3D = GetResolvedKeyFrameValue(i);
		}
		else
		{
			double num3 = 0.0;
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValuePoint3D(defaultOriginValue));
			...
			point3D = GetResolvedKeyFrame(i).InterpolateValue(baseValue, num3);
		}
		if (IsCumulative)
		{
			...
			if (num5 > 0.0)
			{
				point3D = AnimatedTypeHelpers.AddPoint3D(point3D, AnimatedTypeHelpers.ScalePoint3D(GetResolvedKeyFrameValue(num2), num5));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				Point3D from = _keyFrames[num - 1].Value;
				do
				{
					Point3D value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthPoint3D(from, _keyFrames[num].Value);
