...
public class DoubleAnimationUsingKeyFrames : DoubleAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override double GetCurrentValueCore(double defaultOriginValue, double defaultDestinationValue, AnimationClock animationClock)
	{
		...
		else
		{
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValueDouble(defaultOriginValue));
		...
		if (IsCumulative)
		{
			...
			if (num6 > 0.0)
			{
				num3 = AnimatedTypeHelpers.AddDouble(num3, AnimatedTypeHelpers.ScaleDouble(GetResolvedKeyFrameValue(num2), num6));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				double from = _keyFrames[num - 1].Value;
				do
				{
					double value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthDouble(from, _keyFrames[num].Value);
