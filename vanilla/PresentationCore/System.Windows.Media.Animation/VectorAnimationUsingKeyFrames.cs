...
public class VectorAnimationUsingKeyFrames : VectorAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override Vector GetCurrentValueCore(Vector defaultOriginValue, Vector defaultDestinationValue, AnimationClock animationClock)
	{
		...
		if (i == num)
		{
			vector = GetResolvedKeyFrameValue(num2);
		}
		else if (value == _sortedResolvedKeyFrames[i]._resolvedKeyTime)
		{
			vector = GetResolvedKeyFrameValue(i);
		}
		else
		{
			double num3 = 0.0;
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValueVector(defaultOriginValue));
			...
			vector = GetResolvedKeyFrame(i).InterpolateValue(baseValue, num3);
		}
		if (IsCumulative)
		{
			...
			if (num5 > 0.0)
			{
				vector = AnimatedTypeHelpers.AddVector(vector, AnimatedTypeHelpers.ScaleVector(GetResolvedKeyFrameValue(num2), num5));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				Vector from = _keyFrames[num - 1].Value;
				do
				{
					Vector value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthVector(from, _keyFrames[num].Value);
