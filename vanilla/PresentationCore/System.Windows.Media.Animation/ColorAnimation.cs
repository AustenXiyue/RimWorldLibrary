...
public class ColorAnimation : ColorAnimationBase
{
	...
	protected override Color GetCurrentValueCore(Color defaultOriginValue, Color defaultDestinationValue, AnimationClock animationClock)
	{
		...
		case AnimationType.FromBy:
			color = _keyValues[0];
			color2 = AnimatedTypeHelpers.AddColor(_keyValues[0], _keyValues[1]);
			if (IsAdditive)
			{
				value2 = defaultOriginValue;
				flag = true;
		...
		if (IsCumulative)
		{
			...
			if (num2 > 0.0)
			{
				value = AnimatedTypeHelpers.ScaleColor(AnimatedTypeHelpers.SubtractColor(color2, color), num2);
			}
		}
	...
	{
		ColorAnimation obj = (ColorAnimation)d;
	...
	private static bool ValidateFromToOrByValue(object value)
	{
		...
		if (color.HasValue)
		{
			return AnimatedTypeHelpers.IsValidAnimationValueColor(color.Value);
		}
		return true;
	}
}
