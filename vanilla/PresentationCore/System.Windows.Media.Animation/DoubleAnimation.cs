...
public class DoubleAnimation : DoubleAnimationBase
{
	...
	protected override double GetCurrentValueCore(double defaultOriginValue, double defaultDestinationValue, AnimationClock animationClock)
	{
		...
		case AnimationType.FromBy:
			num2 = _keyValues[0];
			num3 = AnimatedTypeHelpers.AddDouble(_keyValues[0], _keyValues[1]);
			if (IsAdditive)
			{
				value2 = defaultOriginValue;
				flag = true;
		...
		if (IsCumulative)
		{
			...
			if (num4 > 0.0)
			{
				value = AnimatedTypeHelpers.ScaleDouble(AnimatedTypeHelpers.SubtractDouble(num3, num2), num4);
			}
		}
	...
	{
		DoubleAnimation obj = (DoubleAnimation)d;
	...
	private static bool ValidateFromToOrByValue(object value)
	{
		...
		if (num.HasValue)
		{
			return AnimatedTypeHelpers.IsValidAnimationValueDouble(num.Value);
		}
		return true;
	}
}
