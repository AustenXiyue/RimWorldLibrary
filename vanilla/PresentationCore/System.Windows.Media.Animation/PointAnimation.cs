...
public class PointAnimation : PointAnimationBase
{
	...
	protected override Point GetCurrentValueCore(Point defaultOriginValue, Point defaultDestinationValue, AnimationClock animationClock)
	{
		...
		case AnimationType.FromBy:
			point = _keyValues[0];
			point2 = AnimatedTypeHelpers.AddPoint(_keyValues[0], _keyValues[1]);
			if (IsAdditive)
			{
				value2 = defaultOriginValue;
				flag = true;
		...
		if (IsCumulative)
		{
			...
			if (num2 > 0.0)
			{
				value = AnimatedTypeHelpers.ScalePoint(AnimatedTypeHelpers.SubtractPoint(point2, point), num2);
			}
		}
	...
	{
		PointAnimation obj = (PointAnimation)d;
	...
	private static bool ValidateFromToOrByValue(object value)
	{
		...
		if (point.HasValue)
		{
			return AnimatedTypeHelpers.IsValidAnimationValuePoint(point.Value);
		}
		return true;
	}
}
