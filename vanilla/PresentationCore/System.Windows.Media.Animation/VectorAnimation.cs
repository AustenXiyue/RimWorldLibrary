...
public class VectorAnimation : VectorAnimationBase
{
	...
	protected override Vector GetCurrentValueCore(Vector defaultOriginValue, Vector defaultDestinationValue, AnimationClock animationClock)
	{
		...
		case AnimationType.FromBy:
			vector = _keyValues[0];
			vector2 = AnimatedTypeHelpers.AddVector(_keyValues[0], _keyValues[1]);
			if (IsAdditive)
			{
				value2 = defaultOriginValue;
				flag = true;
		...
		if (IsCumulative)
		{
			...
			if (num2 > 0.0)
			{
				value = AnimatedTypeHelpers.ScaleVector(AnimatedTypeHelpers.SubtractVector(vector2, vector), num2);
			}
		}
	...
	{
		VectorAnimation obj = (VectorAnimation)d;
	...
	private static bool ValidateFromToOrByValue(object value)
	{
		...
		if (vector.HasValue)
		{
			return AnimatedTypeHelpers.IsValidAnimationValueVector(vector.Value);
		}
		return true;
	}
}
