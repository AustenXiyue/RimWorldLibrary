...
public class DecimalAnimationUsingKeyFrames : DecimalAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override decimal GetCurrentValueCore(decimal defaultOriginValue, decimal defaultDestinationValue, AnimationClock animationClock)
	{
		...
		else
		{
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValueDecimal(defaultOriginValue));
		...
		if (IsCumulative)
		{
			...
			if (num6 > 0.0)
			{
				num3 = AnimatedTypeHelpers.AddDecimal(num3, AnimatedTypeHelpers.ScaleDecimal(GetResolvedKeyFrameValue(num2), num6));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				decimal from = _keyFrames[num - 1].Value;
				do
				{
					decimal value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthDecimal(from, _keyFrames[num].Value);
