...
public class SingleAnimationUsingKeyFrames : SingleAnimationBase, IKeyFrameAnimation, IAddChild
{
	...
	protected sealed override float GetCurrentValueCore(float defaultOriginValue, float defaultDestinationValue, AnimationClock animationClock)
	{
		...
		else
		{
			...
			if (i == 0)
			{
				baseValue = ((!IsAdditive) ? defaultOriginValue : AnimatedTypeHelpers.GetZeroValueSingle(defaultOriginValue));
		...
		if (IsCumulative)
		{
			...
			if (num6 > 0.0)
			{
				num3 = AnimatedTypeHelpers.AddSingle(num3, AnimatedTypeHelpers.ScaleSingle(GetResolvedKeyFrameValue(num2), num6));
			}
		}
		if (IsAdditive)
		{
	...
	private void ResolvePacedKeyTimes()
	{
		...
		do
		{
			if (_keyFrames[num].KeyTime.Type == KeyTimeType.Paced)
			{
				...
				double num4 = 0.0;
				float from = _keyFrames[num - 1].Value;
				do
				{
					float value = _keyFrames[num].Value;
				...
				num4 += AnimatedTypeHelpers.GetSegmentLengthSingle(from, _keyFrames[num].Value);
