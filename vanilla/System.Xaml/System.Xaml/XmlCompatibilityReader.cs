...
internal sealed class XmlCompatibilityReader : XmlWrappingReader
{
		...
		public void Verify()
		{
			if (_processContents != null)
			{
				foreach (string key in _processContents.Keys)
				{
					...
					{
						_reader.Error(System.SR.XCRNSProcessContentNotIgnorable, key);
					}
				}
			}
			if (_preserveElements != null)
			{
				foreach (string key2 in _preserveElements.Keys)
				{
					...
					{
						_reader.Error(System.SR.XCRNSPreserveNotIgnorable, key2);
					}
				}
			}
			if (_preserveAttributes == null)
			...
			foreach (string key3 in _preserveAttributes.Keys)
			{
				...
				{
					_reader.Error(System.SR.XCRNSPreserveNotIgnorable, key3);
				}
			}
		}
	}
	...
	private class ProcessContentSet
	{
		...
		public void Add(string elementName)
		{
			if (ShouldProcessContent(elementName))
			{
				if (elementName == "*")
				{
					_reader.Error(System.SR.XCRDuplicateWildcardProcessContent, _namespaceName);
				}
				else
				{
					_reader.Error(System.SR.XCRDuplicateProcessContent, _namespaceName, elementName);
				}
			}
			if (elementName == "*")
			{
				if (_names != null)
				{
					_reader.Error(System.SR.XCRInvalidProcessContent, _namespaceName);
				}
				else
				{
					_all = true;
	...
	private class PreserveItemSet
	{
		...
		public void Add(string itemName)
		{
			if (ShouldPreserveItem(itemName))
			{
				if (itemName == "*")
				{
					_reader.Error(System.SR.XCRDuplicateWildcardPreserve, _namespaceName);
				}
				else
				{
					_reader.Error(System.SR.XCRDuplicatePreserve, itemName, _namespaceName);
				}
			}
			if (itemName == "*")
			{
				if (_names != null)
				{
					_reader.Error(System.SR.XCRInvalidPreserve, _namespaceName);
				}
				else
				{
					_all = true;
	...
	private bool ReadStartElement(ref bool more)
	{
		...
		if ((object)namespaceURI == CompatibilityUri)
		{
			...
			{
				Error(System.SR.XCRUnknownCompatElement, localName);
			}
		...
		else
		{
			...
			else
			{
				...
				{
					Error(System.SR.XCRInvalidACChild, base.Reader.Name);
				}
				result = true;
			}
		}
	...
	private bool ReadEndElement(ref bool more)
	{
		...
		if ((object)namespaceURI == CompatibilityUri)
		{
			...
			{
				Error(System.SR.XCRChoiceNotFound);
			}
			_depthOffset--;
			PopScope();
			more = base.Reader.Read();
			...
			if (_namespaceCallback(namespaceName, out var newXmlNamespace))
			{
				...
				else
				{
					if (!_namespaceMap.TryGetValue(newXmlNamespace, out var value))
					{
						...
						{
							Error(System.SR.XCRCompatCycle, newXmlNamespace);
						}
	...
	private IEnumerable<NamespaceElementPair> ParseContentToNamespaceElementPair(string content, string callerContext)
	{
		...
		foreach (string text in array)
		{
			if (!string.IsNullOrEmpty(text))
			{
				...
				{
					Error(System.SR.XCRInvalidFormat, callerContext);
				}
				...
				if (text4 == null)
				{
					Error(System.SR.XCRUndefinedPrefix, text2);
				}
				else if (text3 != "*" && !XmlReader.IsName(text3))
				{
					Error(System.SR.XCRInvalidXMLName, text);
				}
				else
				{
	...
	private IEnumerable<string> PrefixesToNamespaces(string prefixes)
	{
		...
		foreach (string text in array)
		{
			if (!string.IsNullOrEmpty(text))
			{
				...
				if (text2 == null)
				{
					Error(System.SR.XCRUndefinedPrefix, text);
				}
				else
				{
					yield return text2;
		...
		do
		{
			...
			if (ShouldIgnoreNamespace(namespaceURI))
			{
				if ((object)namespaceURI == CompatibilityUri)
				{
					...
					{
						Error(System.SR.XCRUnknownCompatAttrib, localName);
					}
					value(elementDepth);
				}
	...
	private void HandleAlternateContent(int elementDepth, ref bool more)
	{
		if (Scope.InAlternateContent)
		{
			Error(System.SR.Format(System.SR.XCRInvalidACChild, base.Reader.Name));
		}
	...
	private void HandleChoice(int elementDepth, ref bool more)
	{
		if (!Scope.InAlternateContent)
		{
			Error(System.SR.XCRChoiceOnlyInAC);
		}
		if (Scope.FallbackSeen)
		{
			Error(System.SR.XCRChoiceAfterFallback);
		}
		string attribute = base.Reader.GetAttribute(Requires);
		if (attribute == null)
		{
			Error(System.SR.XCRRequiresAttribNotFound);
		}
		if (string.IsNullOrEmpty(attribute))
		{
			Error(System.SR.XCRInvalidRequiresAttribute);
		}
		...
		if (AttributeCount != 1)
		{
			...
			string localName = base.Reader.LocalName;
			MoveToElement();
			Error(System.SR.XCRInvalidAttribInElement, localName, Choice);
		}
	...
	private void HandleFallback(int elementDepth, ref bool more)
	{
		if (!Scope.InAlternateContent)
		{
			Error(System.SR.XCRFallbackOnlyInAC);
		}
		if (!Scope.ChoiceSeen)
		{
			Error(System.SR.XCRChoiceNotFound);
		...
		if (Scope.FallbackSeen)
		{
			Error(System.SR.XCRMultipleFallbackFound);
		}
		...
		if (AttributeCount != 0)
		{
			...
			string localName = base.Reader.LocalName;
			MoveToElement();
			Error(System.SR.XCRInvalidAttribInElement, localName, Fallback);
		}
		if (choiceTaken)
		{
			ScanForEndCompatibility(elementDepth);
	...
	private void HandleMustUnderstand(int elementDepth)
	{
		foreach (string item in PrefixesToNamespaces(base.Reader.Value))
		{
			if (!IsNamespaceKnown(item))
			{
				Error(System.SR.XCRMustUnderstandFailed, item);
			}
		}
	}
