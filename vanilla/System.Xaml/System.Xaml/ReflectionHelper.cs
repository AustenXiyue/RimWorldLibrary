	...
	private static string GetCustomAttributeData(CustomAttributeData cad, Type attrType, out Type typeValue, bool allowTypeAlso, bool noArgs, bool zeroArgsAllowed)
	{
		...
		if (cad.Constructor.ReflectedType == attrType)
		{
			...
			if (constructorArguments.Count == 1 && !noArgs)
			{
				...
				if (text == null)
				{
					throw new ArgumentException(System.SR.Format(System.SR.ParserAttributeArgsLow, attrType.Name));
				}
			}
			else
			{
				...
				{
					throw new ArgumentException(System.SR.Format(System.SR.ParserAttributeArgsHigh, attrType.Name));
				}
		...
		if (assembly != null)
		{
			if (assemblyName.Version != null)
			{
				...
				if (!AssemblyName.ReferenceMatchesDefinition(assemblyName, assemblyName2))
				{
					...
					throw new InvalidOperationException(System.SR.Format(System.SR.ParserAssemblyLoadVersionMismatch, p, p2));
				}
			}
		}
		else
