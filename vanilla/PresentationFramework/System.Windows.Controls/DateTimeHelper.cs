...
internal static class DateTimeHelper
{
	...
	{
		if (element.GetValueSource(FrameworkElement.LanguageProperty, null, out var _) != BaseValueSourceInternal.Default)
		{
			return DynamicPropertyReader.GetCultureInfo(element);
		}
	...
	public static string ToDecadeRangeString(int decade, FrameworkElement fe)
	{
		...
		if (dateFormat != null)
		{
			bool num = fe.FlowDirection == FlowDirection.RightToLeft;
			int num2 = (num ? decade : (decade + 9));
			result = (num ? (decade + 9) : decade).ToString(dateFormat) + "-" + num2.ToString(dateFormat);
		}
		return result;
	}
