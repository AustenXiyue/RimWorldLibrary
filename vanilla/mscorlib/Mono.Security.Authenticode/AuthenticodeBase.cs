...
internal class AuthenticodeBase
{
	...
	internal void ReadFirstBlock()
	{
		...
		if (num != 0)
		{
			throw new NotSupportedException(Locale.GetText("Cannot sign non PE files, e.g. .CAB or .MSI files (error {0}).", num));
		}
	}
	...
	internal int ProcessFirstBlock()
	{
		...
		{
			throw new NotSupportedException(string.Format(Locale.GetText("Header size too big (> {0} bytes)."), fileblock.Length));
		}
