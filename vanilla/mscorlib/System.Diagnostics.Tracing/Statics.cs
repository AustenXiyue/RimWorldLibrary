...
internal static class Statics
{
	...
	{
		return propInfo.GetCustomAttributes(attributeType, inherit: false).Length != 0;
	}
	...
	public static TraceLoggingTypeInfo GetTypeInfoInstance(Type dataType, List<Type> recursionCheck)
	{
		...
		return (TraceLoggingTypeInfo)GetDeclaredStaticMethod(typeof(TraceLoggingTypeInfo<>).MakeGenericType(dataType), "GetInstance").Invoke(null, new object[1] { recursionCheck });
	}
	...
	public static TraceLoggingTypeInfo<DataType> CreateDefaultTypeInfo<DataType>(List<Type> recursionCheck)
	{
		...
		else if (IsEnum(typeFromHandle))
		{
			...
			else
			{
				...
				{
					throw new NotSupportedException(Environment.GetResourceString("Enum type {0} underlying type {1} is not supported for serialization.", typeFromHandle.Name, underlyingType.Name));
				}
		...
		else
		{
			...
			if (!(type != null))
			{
				throw new ArgumentException(Environment.GetResourceString("The API supports only anonymous types or types decorated with the EventDataAttribute. Non-compliant type: {0} dataType.", typeFromHandle.Name));
			}
