...
internal class TypeSpec
{
	...
	internal Type Resolve(Func<AssemblyName, Assembly> assemblyResolver, Func<Assembly, string, bool, Type> typeResolver, bool throwOnError, bool ignoreCase)
	{
		...
		if (type == null)
		{
			if (throwOnError)
			{
				throw new TypeLoadException(string.Concat("Could not resolve type '", name, "'"));
			}
			return null;
		}
		if (nested != null)
		{
			foreach (TypeIdentifier item in nested)
			{
				...
				if (nestedType == null)
				{
					if (throwOnError)
					{
						throw new TypeLoadException(string.Concat("Could not resolve type '", item, "'"));
					}
					return null;
				}
				type = nestedType;
		...
		if (generic_params != null)
		{
			...
			{
				Type type2 = generic_params[i].Resolve(assemblyResolver, typeResolver, throwOnError, ignoreCase);
				if (type2 == null)
				{
					if (throwOnError)
					{
						throw new TypeLoadException(string.Concat("Could not resolve type '", generic_params[i].name, "'"));
					}
					return null;
				}
				array[i] = type2;
	...
	private static TypeSpec Parse(string name, ref int p, bool is_recurse, bool allow_aqn)
	{
		...
		if (flag)
		{
			for (; i < name.Length; i++)
			{
				...
				default:
					throw new ArgumentException("Bad type def, can't handle '" + name[i].ToString() + "' at " + i, "typeName");
				}
			}
		}
		p = i;
