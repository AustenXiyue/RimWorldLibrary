...
public class MBDD_CompRangedDeepDrill : MBDD_CompDeepDrill
{
	protected override void TryProducePortion(float yieldPct, Pawn driller = null)
	{
		...
		Messages.Message("DeepDrillExhausted".Translate(Find.ActiveLanguageWorker.Pluralize(DeepDrillUtil.GetBaseResource(parent.Map, parent.Position).label)), parent, MessageTypeDefOf.TaskCompletion);
	...
	{
		return DeepDrillUtil.GetNextResource(parent.Position, parent.Map, out resDef, out countPresent, out cell);
	}
	...
	protected override void UpdateCanDrillState()
	{
		...
		{
			if (DeepDrillUtil.GetBaseResource(parent.Map, parent.Position) != null)
			{
				CanDrillNow = true;
			}
			else
