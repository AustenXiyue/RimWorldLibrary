...
public class ITab_Pawn_Psycasts : ITab
{
	...
	public override bool IsVisible => Find.Selector.SingleSelectedThing is Pawn pawn && pawn.health.hediffSet.HasHediff(VPE_DefOf.VPE_PsycastAbilityImplant) && (pawn.Faction?.IsPlayer ?? false);
	...
	protected override void FillTab()
	{
		...
		if (((Hediff_Level)(object)hediff).level < PsycastsMod.Settings.maxLevel)
		{
			...
			if (devMode)
			{
				...
				if (Widgets.ButtonText(UIUtility.TakeRightPart(ref rect4, 80f), "Dev: Level up", drawBackground: true, doMouseoverSound: true, active: true, null))
				{
		...
		if (smallMode)
		{
			Rect rect5 = new Rect(rect2.x, curHeight, rect2.width / 2f, curHeight2 - curHeight);
		...
		Text.Anchor = TextAnchor.UpperLeft;
		Rect rect6 = listing_Standard.GetRect(48f);
		float num3 = rect2.x;
		foreach (MeditationFocusDef focus in foci)
		{
			if (num3 + 50f >= rect2.width)
			{
				num3 = rect2.x;
				listing_Standard.Gap(3f);
				rect6 = listing_Standard.GetRect(48f);
			}
			Rect inRect = new Rect(num3, rect6.y, 48f, 48f);
		...
		if (!smallMode)
		{
			...
			Rect rect7 = listing_Standard.GetRect(num4);
			Widgets.DrawMenuSection(rect7);
			Rect rect8 = new Rect(0f, 0f, rect7.width - 20f, RequestedPsysetsHeight);
			Widgets.BeginScrollView(rect7.ContractedBy(3f, 6f), ref psysetsScrollPos, rect8);
			DoPsysets(rect8);
		...
		else
		{
			...
			rect3.yMin += 40f;
			Widgets.DrawMenuSection(rect3);
			Rect rect8 = new Rect(0f, 0f, rect3.width - 20f, lastPathsHeight);
			Widgets.BeginScrollView(rect3.ContractedBy(2f), ref pathsScrollPos, rect8);
			DoPaths(rect8);
			Widgets.EndScrollView();
		}
		Text.Font = font;
	...
	private void DoFocus(Rect inRect, MeditationFocusDef def)
	{
		...
		if ((hediff.points >= 1 || devMode) && !flag && (flag2 || devMode) && Widgets.ButtonText(new Rect(inRect.xMax - 13f, inRect.yMax - 13f, 12f, 12f), "▲", drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			if (!devMode)
			{
				hediff.SpentPoints();
	...
	public void DoPsysets(Rect inRect)
	{
		...
		foreach (PsySet item in hediff.psysets.ToList())
		{
			...
			if (Widgets.ButtonText(rect.LeftHalf().RightHalf(), "VPE.Rename".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
			{
				Find.WindowStack.Add(new Dialog_RenamePsyset(item));
			}
			if (Widgets.ButtonText(rect.RightHalf().LeftHalf(), "VPE.Edit".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
			{
				Find.WindowStack.Add(new Dialog_Psyset(item, pawn));
			}
			if (Widgets.ButtonText(rect.RightHalf().RightHalf(), "VPE.Remove".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
			{
				hediff.RemovePsySet(item);
			}
		}
		if (Widgets.ButtonText(listing_Standard.GetRect(70f).LeftHalf().ContractedBy(5f), "VPE.CreatePsyset".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
		...
		foreach (PsycasterPathDef def in pathsByTab[curTab].OrderByDescending(hediff.unlockedPaths.Contains).ThenBy((PsycasterPathDef path) => path.order).ThenBy((PsycasterPathDef path) => path.label))
		{
			...
			else
			{
				...
				if (hediff.points >= 1 || devMode)
				{
					...
					{
						if (Widgets.ButtonText(rect2, "VPE.Unlock".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
						{
							if (!devMode)
							{
								hediff.SpentPoints();
					...
					else
					{
						...
						Widgets.ButtonText(rect2, text, drawBackground: true, doMouseoverSound: true, active: false, null);
						GUI.color = Color.white;
					}
				}
	...
	private void DoAbility(Rect inRect, AbilityDef ability)
	{
		...
		if (unlockable)
		{
			Widgets.DrawStrongHighlight(inRect.ExpandedBy(12f), null);
		}
