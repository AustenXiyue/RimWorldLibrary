...
public class PsychicStatusGizmo : Gizmo
{
	...
	public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
	{
		...
		Gizmo lastMouseoverGizmo = ((MainTabWindow_Inspect)MainButtonDefOf.Inspect.TabWindow).LastMouseoverGizmo;
		...
		{
			if (Math.Abs(num7 - selectedPsyfocusTarget) > float.Epsilon)
			{
				SoundDefOf.DragSlider.PlayOneShotOnCamera();
			}
			selectedPsyfocusTarget = num7;
