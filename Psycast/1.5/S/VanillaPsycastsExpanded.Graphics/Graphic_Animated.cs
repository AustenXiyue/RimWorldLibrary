...
public class Graphic_Animated : Graphic_Collection
{
	...
	private Graphic CurFrame
	{
		get
		{
			...
			return (array != null) ? array[Mathf.FloorToInt(((((float?)Current.Game?.tickManager?.TicksGame) ?? 0f) + (float)offset) / (float)((GraphicData_Animated)data).ticksPerFrame) % subGraphics.Length] : null;
		}
	}
