...
public class Decoy : ThingWithComps, IAttackTarget, ILoadReferenceable
{
	...
	public static bool BestAttackTarget_Prefix(IAttackTargetSearcher searcher, ref IAttackTarget __result)
	{
		...
		List<IAttackTarget> potentialTargetsFor = searcher.Thing.Map.attackTargetsCache.GetPotentialTargetsFor(searcher);
		List<Decoy> list = potentialTargetsFor.OfType<Decoy>().ToList();
		if (list.NullOrEmpty())
		{
			return true;
		}
