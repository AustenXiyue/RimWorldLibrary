...
public class Ability_Wallraise : Ability
{
	...
	public override void Cast(params GlobalTargetInfo[] targets)
	{
		...
		for (int i = 0; i < targets.Length; i++)
		{
			...
			foreach (Thing item3 in list)
			{
				...
				for (int j = 0; j < 9; j++)
				{
					IntVec3 intVec2 = item3.Position + GenRadial.RadialPattern[j];
	...
	public override void DrawHighlight(LocalTargetInfo target)
	{
		...
		GenDraw.DrawFieldEdges(Props.AffectedCells(target, base.pawn.Map).ToList(), ((Ability)this).ValidateTarget(target, false) ? Color.white : Color.red, null);
	}
