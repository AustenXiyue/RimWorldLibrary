...
public class Ability_WorldTeleport : Ability
{
	...
	private IEnumerable<Pawn> PawnsToSkip()
	{
		...
		foreach (Thing thing in GenRadial.RadialDistinctThingsAround(base.pawn.Position, base.pawn.Map, ((Ability)this).GetRadiusForPawn(), useCenter: true))
		{
			Pawn pawn2 = thing as Pawn;
			if (pawn2 != null && !pawn2.Dead && (pawn2.IsColonist || pawn2.IsPrisonerOfColony || (!homeMap && pawn2.RaceProps.Animal && (pawn2.Faction?.IsPlayer ?? false))))
			{
				yield return pawn2;
			}
		}
	...
	private bool ShouldEnterMap(GlobalTargetInfo target)
	{
		...
		return target.WorldObject is MapParent { HasMap: not false } mapParent && (AlliedPawnOnMap(mapParent.Map) != null || mapParent.Map == base.pawn.Map);
	}
	...
	public override bool CanHitTargetTile(GlobalTargetInfo target)
	{
		...
		return (caravan == null || caravan != caravan2) && (ShouldEnterMap(target) || (caravan2 != null && caravan2.Faction == base.pawn.Faction)) && ((Ability)this).CanHitTargetTile(target);
	}
