...
{
	private static readonly FieldRef<World, List<ThingDef>> allNaturalRockDefs = AccessTools.FieldRefAccess<World, List<ThingDef>>("allNaturalRockDefs");

	private static readonly FieldRef<Thing, Graphic> graphicInt = AccessTools.FieldRefAccess<Thing, Graphic>("graphicInt");
	...
	public override void Cast(params GlobalTargetInfo[] targets)
	{
		...
		for (int i = 0; i < targets.Length; i++)
		{
			...
			List<ThingDef> naturalRockDefs = allNaturalRockDefs.Invoke(Find.World);
			...
			foreach (IntVec3 item in GenRadial.RadialCellsAround(globalTargetInfo.Cell, ((Ability)this).GetRadiusForPawn(), useCenter: true))
			{
				foreach (Thing item2 in item.GetThingList(map).ListFullCopy())
				{
					...
					{
						if (item3.building.mineableThing == item2.def)
						{
							Replace(item2, chosenRock.building.mineableThing);
						}
						else if (item3.building.smoothedThing == item2.def)
						{
				...
				TerrainDef terrain2 = terrainGrid.TerrainAt(item);
				terrainGrid.SetTerrain(item, NewTerrain(terrain2));
				terrain2 = terrainGrid.UnderTerrainAt(item);
				if (terrain2 != null)
				{
					terrainGrid.SetUnderTerrain(item, NewTerrain(terrain2));
				}
			}
