...
public class Hediff_IceShield : Hediff_Overlay
{
	...
	{
		base.PostAdd(dinfo);
		List<Hediff> list = ((Hediff)(object)this).pawn.health.hediffSet.hediffs.Where((Hediff x) => x.def == HediffDefOf.Hypothermia || x.def == VPE_DefOf.VFEP_HypothermicSlowdown || x.def == VPE_DefOf.HypothermicSlowdown).ToList();
