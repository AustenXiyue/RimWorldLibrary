...
public class Ability_SnapFreeze : Ability
{
	...
	{
		AbilityExtension_Hediff modExtension = ((Def)(object)base.def).GetModExtension<AbilityExtension_Hediff>();
		if (modExtension.targetOnlyEnemies && target.Thing != null && !target.Thing.HostileTo(base.pawn))
		{
			if (showMessages)
			{
	...
	public static void ApplyHediff(Ability ability, LocalTargetInfo targetInfo)
	{
		...
		if (hediff is HediffWithComps hediffWithComps)
		{
			...
			{
				HediffComp hediffComp = comp;
				HediffComp hediffComp2 = hediffComp;
				HediffComp_Ability val2 = (HediffComp_Ability)(object)((hediffComp2 is HediffComp_Ability) ? hediffComp2 : null);
				if (val2 == null)
				{
					if (hediffComp2 is HediffComp_Disappears hediffComp_Disappears)
					{
		...
		targetInfo.Pawn.health.AddHediff(hediff, null, null);
	}
}
