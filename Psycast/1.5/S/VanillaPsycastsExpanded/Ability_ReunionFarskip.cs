...
public class Ability_ReunionFarskip : Ability
{
	...
	public override void Cast(params GlobalTargetInfo[] targets)
	{
		...
		foreach (Pawn item in livingFamilyMembers)
		{
			...
			item.health.AddHediff(hediff, result, null);
		}
		IEnumerable<Faction> enumerable = (from x in livingFamilyMembers
			select x.Faction into x
			where x != null
			select x).Distinct();
		foreach (Faction item2 in enumerable)
		{
			item2.TryAffectGoodwillWith(base.pawn.Faction, -10, canSendMessage: true, canSendHostilityLetter: true, null, null);
		}
	}
