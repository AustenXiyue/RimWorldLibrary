...
public class Hediff_PsycastAbilities : Hediff_Abilities
{
	...
	public void GainExperience(float experienceGain, bool sendLetter = true)
	{
		if (((Hediff_Level)this).level < PsycastsMod.Settings.maxLevel)
		{
			...
			bool flag = false;
			while (experience >= (float)ExperienceRequiredForLevel(((Hediff_Level)this).level + 1))
			{
	...
	{
		return ((Hediff_Abilities)this).SatisfiesConditionForAbility(abilityDef) || abilityDef.requiredHediff?.minimumLevel <= psylink.level;
	}
	...
	{
		return psysetIndex == psysets.Count || psysets[psysetIndex].Abilities.Contains(ability.def);
	}
	...
	{
		((Hediff_Level)this).Tick();
		IChannelledPsycast channelledPsycast = currentlyChanneling;
		if (channelledPsycast != null && !channelledPsycast.IsActive)
		{
			currentlyChanneling = null;
		}
		if (minHeatGivers.RemoveAll((IMinHeatGiver giver) => !giver.IsActive) > 0)
		{
			RecacheCurStage();
		}
	}
