...
public class Projectile_FrostRay : Projectile
{
	...
	public static Func<Projectile, float> ArcHeightFactor = (Func<Projectile, float>)Delegate.CreateDelegate(typeof(Func<Projectile, float>), null, AccessTools.Method(typeof(Projectile), "get_ArcHeightFactor", (Type[])null, (Type[])null));
	...
	public override void Tick()
	{
		...
		{
			Hediff firstHediffOfDef = pawn.health.hediffSet.GetFirstHediffOfDef(VPE_DefOf.VPE_FrostRay);
			if (firstHediffOfDef == null || pawn.Downed || pawn.Dead)
			{
				Destroy();
				return;
			}
