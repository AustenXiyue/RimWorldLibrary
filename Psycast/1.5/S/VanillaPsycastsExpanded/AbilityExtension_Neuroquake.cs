...
public class AbilityExtension_Neuroquake : AbilityExtension_AbilityMod
{
	...
	public override void Cast(GlobalTargetInfo[] targets, Ability ability)
	{
		...
		foreach (Pawn item2 in giveMentalStateTo)
		{
			...
			AbilityExtension_GiveMentalState.TryGiveMentalStateWithDuration(mentalStateDef, item2, ability, StatDefOf.PsychicSensitivity, durationScalesWithCaster: false);
		...
		affectedFactions.Clear();
		giveMentalStateTo.Clear();
		Hediff hediff = HediffMaker.MakeHediff(VPE_DefOf.PsychicComa, ability.pawn);
		hediff.TryGetComp<HediffComp_Disappears>().ticksToDisappear = (int)(300000f / ability.pawn.GetStatValue(StatDefOf.PsychicSensitivity));
