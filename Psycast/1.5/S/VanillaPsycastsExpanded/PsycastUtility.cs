...
public static class PsycastUtility
{
	...
	{
		return def != null && (def == VPE_DefOf.VPE_Eltex || (def.costList != null && def.costList.Any((ThingDefCountClass x) => x.thingDef == VPE_DefOf.VPE_Eltex)) || eltexThings.Contains(def));
	}
	...
	[DebugAction("Pawns", "Reset Psycasts", true, false, false, false, 0, false, actionType = DebugActionType.ToolMapForPawns, allowedGameStates = AllowedGameStates.PlayingOnMap)]
