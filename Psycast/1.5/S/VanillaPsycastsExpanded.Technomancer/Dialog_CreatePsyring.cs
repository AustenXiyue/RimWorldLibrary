...
public class Dialog_CreatePsyring : Window
{
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		foreach (AbilityDef possibleAbility in possibleAbilities)
		{
			...
			Rect position = UIUtility.TakeLeftPart(ref rect, 64f);
			rect.xMin += 5f;
			GUI.DrawTexture(position, Command.BGTex);
			GUI.DrawTexture(position, possibleAbility.icon);
			Widgets.Label(UIUtility.TakeTopPart(ref rect, 20f), ((Def)(object)possibleAbility).LabelCap);
			if (Widgets.ButtonText(UIUtility.TakeBottomPart(ref rect, 20f), "VPE.CreatePsyringButton".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
			{
