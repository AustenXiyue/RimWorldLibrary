...
public class Verb_Shoot_Fortress : Verb_Shoot
{
	...
	protected override bool TryCastShot()
	{
		...
		for (int i = 0; i < 20; i++)
		{
			AncotFleckMaker.CustomFleckThrow(building_TurretGunFortress.Map, FleckDefOf.AirPuff, building_TurretGunFortress.Position.ToVector3Shifted(), new Color(0.92f, 0.91f, 0.76f), normalized + new Vector3(Rand.Range(-0.05f, 0.05f), 0f, Rand.Range(-0.05f, 0.05f)), Rand.Range(1f, 4f), 0f, curRotation + 180f + Rand.Range(-30f, 30f), Rand.Range(4f, 12f));
		}
		for (int j = 0; j < 8; j++)
		{
			AncotFleckMaker.CustomFleckThrow(building_TurretGunFortress.Map, RimWorld.FleckDefOf.MicroSparksFast, building_TurretGunFortress.Position.ToVector3Shifted(), new Color(0.92f, 0.91f, 0.76f), normalized, Rand.Range(1f, 4f), 0f, curRotation + Rand.Range(-10f, 10f), Rand.Range(1f, 3f));
		}
