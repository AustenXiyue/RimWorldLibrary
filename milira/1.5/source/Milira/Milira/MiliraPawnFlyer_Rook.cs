...
public class MiliraPawnFlyer_Rook : MiliraPawnFlyer
{
	...
	public VerbTracker verbTracker => (base.FlyingPawn.equipment.Primary == null) ? base.FlyingPawn.verbTracker : weapon.TryGetComp<CompEquippable>()?.verbTracker;
	...
	private void ChargeDamage()
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			if (list[i] is Pawn)
			{
				...
				{
					AncotUtility.DoDamage(pawn, DamageDefOf.Stab, damageAmount, armorPenetration, base.FlyingPawn);
					if (!pawn.Dead)
					{
			...
			if (list[i] is Building)
			{
				...
				{
					AncotUtility.DoDamage(building, DamageDefOf.Stab, damageAmount, armorPenetration, base.FlyingPawn);
				}
			}
		}
	}
