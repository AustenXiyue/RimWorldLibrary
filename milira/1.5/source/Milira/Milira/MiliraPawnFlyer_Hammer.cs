...
public class MiliraPawnFlyer_Hammer : MiliraPawnFlyer
{
	...
	public override void LandingEffects()
	{
		...
		foreach (IntVec3 item in AffectedCells(intVec))
		{
			...
			GenExplosion.DoExplosion(item, base.Map, 0.8f, MiliraDefOf.Milira_PlasmaBomb, base.FlyingPawn, (int)damageAmount, armorPenetration, null, null, null, null, null, 0f, 1, null, applyDamageToExplosionCellsNeighbors: false, null, 0f, 1, 0f, damageFalloff: false, null, null, null);
		}
		...
		for (int i = 0; i < list.Count; i++)
		{
			if (list[i] is Pawn)
			{
				...
				{
					AncotUtility.DoDamage(pawn, MiliraDefOf.Milira_PlasmaBomb, damageAmount, armorPenetration, base.FlyingPawn);
					AncotUtility.DoDamage(pawn, DamageDefOf.Stun, damageAmount / 5f, armorPenetration, base.FlyingPawn);
					if (!pawn.Dead)
					{
