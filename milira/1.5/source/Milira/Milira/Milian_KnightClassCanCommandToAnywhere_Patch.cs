...
public static class Milian_KnightClassCanCommandToAnywhere_Patch
{
	...
	public static bool InPawnIVCommandRange(Pawn mech, LocalTargetInfo target)
	{
		...
		List<Pawn> list = currentMap.mapPawns.SpawnedColonyMechs.Where((Pawn p) => p.def.defName == "Milian_Mechanoid_PawnIV").ToList();
		if (!list.NullOrEmpty())
		{
			foreach (Pawn item in list)
			{
				...
				if (!item.Downed && CanCommandTo(item, target))
				{
