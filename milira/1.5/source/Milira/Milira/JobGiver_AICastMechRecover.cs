...
public class JobGiver_AICastMechRecover : JobGiver_AICastAbility
{
	...
	protected override LocalTargetInfo GetTarget(Pawn caster, Ability ability)
	{
		...
		{
			if (item.health.summaryHealth.SummaryHealthPercent < 0.8f && !item.HostileTo(caster.Faction) && item.Position.InHorDistOf(caster.Position, 15f) && ability.CanApplyOn(new LocalTargetInfo(item)) && item.Map.pathFinder.FindPath(caster.Position, item, TraverseParms.For(caster)).Found)
			{
