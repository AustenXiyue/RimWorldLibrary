...
public class JobDriver_MilianRepair : JobDriver
{
	...
	protected override IEnumerable<Toil> MakeNewToils()
	{
		...
		this.FailOn(() => RepairObject.Position.DistanceTo(pawn.Position) > Range);
		if (!Remote)
		{
		...
		{
			ticksToNextRepair -= RepairSpeedFactor();
			if (ticksToNextRepair <= 0f)
			{
				...
				if (RepairObject is Pawn)
				{
					...
					{
						pawn.needs.energy.CurLevel -= energyCostFactor_Target * pawn.GetStatValue(StatDefOf.MechEnergyLossPerHP);
					}
		...
		if (RepairObject is Pawn)
		{
			...
			toil.AddEndCondition(() => MechRepairUtility.CanRepair(pawn2) ? JobCondition.Ongoing : JobCondition.Succeeded);
		}
		else
		{
